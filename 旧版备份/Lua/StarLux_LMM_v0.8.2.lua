-- StarLux 落地率插件 v0.8.2 测试版
-- 适用于 X-Plane 12 + FlyWithLua
-- v0.8.2 调整 Nice/Stable 配色，并新增默认关闭的完整数学日志选项。
...
local function write_landing_log()
    ...
    file:write("StarLux 落地率插件 v0.8.2 - 单次落地记录\n")
    file:write("======================================================================\n\n")
    ...
    file:write("评分说明: v0.8.2 测试阶段的拉平曲率不参与评分。\n")
...
if logMsg then
    logMsg(string.format(
        "[StarLux LMM] v0.8.2 loaded successfully with %d direct XPLM DataRefs.",
        #LMM_DATAREF_SPECS
    ))
end
