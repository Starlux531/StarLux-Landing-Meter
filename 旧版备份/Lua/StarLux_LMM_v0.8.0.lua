-- StarLux 落地率插件 v0.8.0 测试版
-- 适用于 X-Plane 12 + FlyWithLua
-- v0.8.0 新增100英尺拉平曲率、弹跳识别、第二次触地记录和分层报告。
...
local function write_landing_log()
    ...
    file:write("StarLux 落地率插件 v0.8.0 - 单次落地记录\n")
    file:write("======================================================================\n\n")
    ...
    file:write("评分说明: v0.8.0 测试阶段的拉平曲率不参与评分。\n")
...
if logMsg then
    logMsg(string.format(
        "[StarLux LMM] v0.8.0 loaded successfully with %d direct XPLM DataRefs.",
        #LMM_DATAREF_SPECS
    ))
end
