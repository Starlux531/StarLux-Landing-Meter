-- StarLux 落地率插件 v0.9.1 测试版
-- 适用于 X-Plane 12 + FlyWithLua
-- v0.9.1 移除设置窗口内的记录入口，并新增 0 fpm/50 ft 参考线、风向、姿态和可选跑道几何可视化。
...
function log_tools.dominant_factor_text(report)
    if report.legacy then
        return "旧版记录保留当时的评分结论，不使用 0.9.1 阈值重新评级。"
    end
...
function log_tools.write_viewer_html(report)
    ...
    local relative_wind_deg = nil
    if report.wind_from_deg ~= nil and report.heading_deg ~= nil then
        relative_wind_deg = normalize_deg(report.wind_from_deg - report.heading_deg)
        if relative_wind_deg > 180 then relative_wind_deg = relative_wind_deg - 360 end
    end
    local runway_marker_percent = nil
    if report.runway_length_m ~= nil
        and report.runway_length_m > 0
        and report.touchdown_from_threshold_m ~= nil then
        runway_marker_percent = math.max(
            3,
            math.min(97, report.touchdown_from_threshold_m / report.runway_length_m * 100)
        )
    end
...
.context-toggle{width:100%;margin-top:16px;border:1px solid #3a6076;background:#17364a;color:#eef5f7;border-radius:10px;padding:10px 14px;font-weight:700;cursor:pointer}
.context-toggle:hover{background:#1d435a}
.context-detail{display:grid;grid-template-columns:1fr 1fr 1.2fr;gap:14px;margin-top:16px}
.context-detail[hidden]{display:none}
.viz-card{border:1px solid var(--line);background:linear-gradient(180deg,rgba(18,43,60,.96),rgba(13,32,45,.96));border-radius:16px;padding:18px;min-height:285px}
.viz-title{font-size:16px;font-weight:800;margin-bottom:4px}.viz-sub{font-size:11px;color:var(--muted);line-height:1.6;margin-bottom:10px}
.viz-caption{text-align:center;color:#c5d6de;font-size:12px;line-height:1.6;margin-top:8px}
.wind-board,.pose-board{height:180px;border:1px solid #28495c;border-radius:13px;background:#0a1c28;position:relative;overflow:hidden}
.wind-board{width:180px;border-radius:50%;margin:0 auto}
.wind-board:before,.wind-board:after{content:"";position:absolute;background:#27495d}
.wind-board:before{width:1px;top:14px;bottom:14px;left:50%}.wind-board:after{height:1px;left:14px;right:14px;top:50%}
.plane-top{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%) rotate(-90deg);font-size:57px;color:#dce9ef;z-index:3;text-shadow:0 0 1px #8bb1c5}
.nose-label{position:absolute;left:50%;top:7px;transform:translateX(-50%);font-size:10px;color:#9fb2bd;z-index:4}
.wind-arrow{position:absolute;left:calc(50% - 2px);top:20px;width:4px;height:49px;background:#53b7df;border-radius:4px;transform-origin:2px 70px;z-index:4}
.wind-arrow:after{content:"";position:absolute;left:-5px;bottom:-5px;border-left:7px solid transparent;border-right:7px solid transparent;border-top:11px solid #53b7df}
.pose-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.pose-mini{background:#0b1b27;border:1px solid #274557;border-radius:11px;padding:8px;text-align:center}.pose-mini b{font-size:11px}
.pose-board{height:160px;border:0;border-radius:7px;margin-top:5px}
.horizon{position:absolute;left:9%;right:9%;top:50%;height:2px;background:#3e657a}
.plane-front{position:absolute;left:50%;top:50%;width:72%;height:34px;transform:translate(-50%,-50%)}
.plane-front:before{content:"";position:absolute;left:0;right:0;top:15px;height:5px;background:#dce9ef;border:1px solid #8bb1c5;border-radius:50%}
.plane-front:after{content:"";position:absolute;left:46%;top:0;width:8%;height:34px;background:#dce9ef;border:1px solid #8bb1c5;clip-path:polygon(50% 0,100% 100%,50% 78%,0 100%)}
.airflow{position:absolute;left:8%;right:8%;top:54%;border-top:2px dashed #53b7df}
.airflow:before{content:"";position:absolute;left:-2px;top:-6px;border-right:8px solid #53b7df;border-top:5px solid transparent;border-bottom:5px solid transparent}
.plane-side{position:absolute;left:13%;top:47%;width:75%;height:24px;background:#dce9ef;border:1px solid #8bb1c5;clip-path:polygon(0 55%,78% 12%,96% 28%,100% 50%,80% 72%,10% 83%)}
.runway-wrap{height:185px;display:flex;align-items:center;justify-content:center;padding:8px}
.runway-strip{position:relative;width:92%;height:62px;border:2px solid #8a9aa3;border-radius:4px;background:#26343d;box-shadow:0 12px 30px rgba(0,0,0,.35)}
.runway-strip:before{content:"";position:absolute;left:5%;right:5%;top:29px;border-top:3px dashed #d8e0e4}
.runway-threshold{position:absolute;left:3%;top:7px;bottom:7px;width:11%;background:repeating-linear-gradient(90deg,#dfe7ea 0 3px,transparent 3px 7px)}
.touch-marker{position:absolute;top:-19px;width:4px;height:96px;background:#ffca58;box-shadow:0 0 0 3px rgba(255,202,88,.22)}
.touch-marker:before{content:"触地点";position:absolute;top:-21px;left:50%;transform:translateX(-50%);white-space:nowrap;font-size:10px;color:#ffdb83}
.geometry-empty{height:185px;display:flex;align-items:center;justify-content:center;text-align:center;color:var(--muted);border:1px dashed #355268;border-radius:12px;line-height:1.7;padding:18px}
.pose-board.geometry-empty{height:160px;padding:10px}
...
@media(max-width:850px){.grid{grid-template-columns:1fr 1fr}.layout{grid-template-columns:1fr}.hero-grid{grid-template-columns:1fr}.status{min-width:0}.facts{grid-template-columns:1fr}.context-detail{grid-template-columns:1fr}}
@media(max-width:520px){.grid{grid-template-columns:1fr}.shell{padding:18px 12px 45px}h1{font-size:24px}.hero{padding:20px}}
</style>
</head>
<body>
<main class="shell">
<div class="topbar"><div class="brand">STARLUX LANDING METRICS MONITOR · 0.9.1</div><div class="privacy">本地生成 · 数据未上传</div></div>
<section class="hero">
...
<aside class="section"><h2>着陆上下文</h2><div class="lead">帮助快速还原这次接地所处的环境。</div><div class="facts">]=])
    ...
    local facts = {
        ...
        { "距机场参考点", report.airport_distance_text ~= "" and report.airport_distance_text or "--" },
    ...
    file:write([=[</div><button class="context-toggle" id="contextToggle" type="button" aria-expanded="false">显示着陆上下文详情</button></aside></div>
<section class="context-detail" id="contextDetails" hidden>
<article class="viz-card"><div class="viz-title">相对风向</div><div class="viz-sub">飞机固定朝向页面上方；箭头表示气流从哪里吹向飞机。</div>]=])
    if relative_wind_deg ~= nil then
        file:write([=[<div class="wind-board" role="img" aria-label="相对风向示意"><span class="nose-label">机头方向</span><span class="plane-top">✈</span><i class="wind-arrow" style="transform:rotate(]=])
        file:write(string.format("%.2f", relative_wind_deg))
        file:write([=[deg)"></i></div><div class="viz-caption">]=])
        file:write(log_tools.html_escape(string.format(
            "风来自 %03d° / %.0f kt；相对机头 %+d°",
            round_num(normalize_deg(report.wind_from_deg)),
            report.wind_speed_kts or 0,
            round_num(relative_wind_deg)
        )))
        file:write("</div>")
    else
        file:write('<div class="wind-board geometry-empty">当前日志缺少飞机航向或风向，无法绘制可靠的相对风向。</div>')
    end
    file:write([=[</article>
<article class="viz-card"><div class="viz-title">接地姿态</div><div class="viz-sub">用简化机体示意横滚与迎角，便于快速判断接地姿态。</div><div class="pose-grid">]=])
    if report.roll_deg ~= nil then
        file:write([=[<div class="pose-mini"><b>横滚（正视）</b><div class="pose-board" role="img" aria-label="横滚姿态示意"><i class="horizon"></i><i class="plane-front" style="transform:translate(-50%,-50%) rotate(]=])
        file:write(string.format("%.2f", report.roll_deg))
        file:write([=[deg)"></i></div><div class="viz-caption">]=])
        file:write(log_tools.html_escape(report.roll_text ~= "" and report.roll_text or string.format("%.1f°", report.roll_deg)))
        file:write("</div></div>")
    else
        file:write('<div class="pose-mini"><b>横滚（正视）</b><div class="pose-board geometry-empty">无横滚数据</div></div>')
    end
    if report.aoa_deg ~= nil then
        file:write([=[<div class="pose-mini"><b>迎角（侧视）</b><div class="pose-board" role="img" aria-label="迎角姿态示意"><i class="airflow"></i><i class="plane-side" style="transform:rotate(]=])
        file:write(string.format("%.2f", -report.aoa_deg))
        file:write([=[deg)"></i></div><div class="viz-caption">]=])
        file:write(log_tools.html_escape(report.aoa_text ~= "" and report.aoa_text or string.format("%.1f°", report.aoa_deg)))
        file:write("</div></div>")
    else
        file:write('<div class="pose-mini"><b>迎角（侧视）</b><div class="pose-board geometry-empty">无迎角数据</div></div>')
    end
    file:write([=[</div><div class="viz-caption">该图为快速复盘示意，不代替飞机姿态仪或飞行数据记录器。</div></article>
<article class="viz-card"><div class="viz-title">跑道触地点</div><div class="viz-sub">只有日志同时包含真实跑道长度与“距跑道入口”数据时才绘制，避免伪精确。</div>]=])
    if runway_marker_percent ~= nil then
        file:write([=[<div class="runway-wrap"><div class="runway-strip"><div class="runway-threshold"></div><div class="touch-marker" style="left:calc(]=])
        file:write(string.format("%.2f", runway_marker_percent))
        file:write([=[% - 2px)"></div></div></div><div class="viz-caption">]=])
        file:write(log_tools.html_escape(string.format(
            "RWY%s · 跑道 %.0f m · 入口后约 %.0f m（%.1f%%）",
            report.runway,
            report.runway_length_m,
            report.touchdown_from_threshold_m,
            report.touchdown_from_threshold_m / report.runway_length_m * 100
        )))
        file:write("</div>")
    else
        file:write([=[<div class="geometry-empty">当前日志没有真实跑道长度和入口距离。为避免误导，不使用“距机场参考点”代替跑道触地点。</div><div class="viz-caption">]=])
        file:write(log_tools.html_escape(
            report.airport_distance_text ~= ""
                and ("已知距机场参考点：" .. report.airport_distance_text .. "（仅作机场识别参考）")
                or "未提供机场参考距离"
        ))
        file:write("</div>")
    end
    file:write([=[</article></section>
<div class="layout">
...
<footer>StarLux LMM 0.9.1 本地可视化 · 页面由插件在用户电脑上即时生成<br>本报告仅用于飞行模拟复盘，不替代航司、维修或适航判定。</footer>
</main>
<script>
const flareData = []=])
...
function drawFlare(){
 ...
 const rawSpan=Math.max(60,ymax-ymin);
 ymax+=rawSpan*.06;ymin-=rawSpan*.10;
 ...
 if(ymin<=0&&ymax>=0){const zeroY=Y(0);c.save();c.setLineDash([8,6]);c.strokeStyle="#d7f3ff";c.lineWidth=2;c.beginPath();c.moveTo(p.l,zeroY);c.lineTo(W-p.r,zeroY);c.stroke();c.setLineDash([]);c.fillStyle="#d7f3ff";c.textAlign="left";c.font="700 11px Microsoft YaHei";c.fillText("0 fpm 参照线",p.l+8,Math.max(p.t+12,zeroY-7));c.restore()}
 let fiftyFtTime=null;
 for(let i=1;i<flareData.length;i++){const a=flareData[i-1],b=flareData[i];if(Number.isFinite(a.ra)&&Number.isFinite(b.ra)&&a.ra>=50&&b.ra<=50){const span=a.ra-b.ra,ratio=span>0?(a.ra-50)/span:0;fiftyFtTime=a.t+(b.t-a.t)*Math.max(0,Math.min(1,ratio));break}}
 if(fiftyFtTime!==null){const fiftyX=X(fiftyFtTime),labelOnRight=fiftyX<p.l+pw*.78;c.save();c.setLineDash([5,5]);c.strokeStyle="#ffca58";c.lineWidth=2;c.beginPath();c.moveTo(fiftyX,p.t);c.lineTo(fiftyX,p.t+ph);c.stroke();c.setLineDash([]);c.fillStyle="#ffdc8a";c.font="700 11px Microsoft YaHei";c.textAlign=labelOnRight?"left":"right";c.fillText("RA 50 ft 入口",fiftyX+(labelOnRight?7:-7),p.t+14);c.restore()}
...
const contextToggle=document.getElementById("contextToggle"),contextDetails=document.getElementById("contextDetails");
if(contextToggle&&contextDetails){contextToggle.addEventListener("click",()=>{const opening=contextDetails.hidden;contextDetails.hidden=!opening;contextToggle.setAttribute("aria-expanded",opening?"true":"false");contextToggle.textContent=opening?"收起着陆上下文详情":"显示着陆上下文详情"})}
</script></body></html>]=])
...
local function write_landing_log()
    ...
    file:write("\239\187\191")
    file:write("StarLux 落地率插件 v0.9.1 - 单次落地记录\n")
    file:write("======================================================================\n\n")
    ...
    file:write("评分说明: v0.9.1 测试阶段的拉平曲率不参与评分。\n")
...
if logMsg then
    logMsg(string.format(
        "[StarLux LMM] v0.9.1 loaded successfully with %d direct XPLM DataRefs.",
        #LMM_DATAREF_SPECS
    ))
end
