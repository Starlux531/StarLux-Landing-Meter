...
local FLARE_CONFIG = {
    ...
    -- 首次接地后持续接地满 2 秒才结束；弹跳会重置连续接地计时。
    post_touch_ground_seconds = 2.0,
    -- 防止触地后持续离地、触地信号异常或触地复飞导致轨迹永不结束。
    post_touch_max_seconds = 8.0,
...
local LMM_DATAREF_SPECS = {
    ...
    { key = "framerate_period_sec", path = "sim/time/framerate_period", kind = "float" },
    ...
    { key = "paused", path = "sim/time/paused", kind = "int" },
    { key = "heading_deg_true", path = "sim/flightmodel/position/psi", kind = "float" }
}
...
-- v1.1.5 起操纵链全部是可选观测项：缺失只影响操纵图，不阻止核心落地算法工作。
-- 数组索引采用 X-Plane 官方常规飞机布局：主翼 0/1、平尾 8/9、垂尾 10/11。
local LMM_CONTROL_REFS = {
    inputs = {
        pitch = {
            candidates = {
                { id = "joystick", path = "sim/joystick/yoke_pitch_ratio", value_field = "pitch_input_ratio", valid_field = "pitch_input_valid" },
                { id = "cockpit", path = "sim/cockpit2/controls/yoke_pitch_ratio", value_field = "pitch_input_cockpit_ratio", valid_field = "pitch_input_cockpit_valid" },
                { id = "flight_control", path = "sim/joystick/FC_ptch", value_field = "pitch_input_flight_control_ratio", valid_field = "pitch_input_flight_control_valid" }
            }
        },
        roll = {
            candidates = {
                { id = "joystick", path = "sim/joystick/yoke_roll_ratio", value_field = "roll_input_ratio", valid_field = "roll_input_valid" },
                { id = "cockpit", path = "sim/cockpit2/controls/yoke_roll_ratio", value_field = "roll_input_cockpit_ratio", valid_field = "roll_input_cockpit_valid" },
                { id = "flight_control", path = "sim/joystick/FC_roll", value_field = "roll_input_flight_control_ratio", valid_field = "roll_input_flight_control_valid" }
            }
        },
        yaw = {
            candidates = {
                { id = "joystick", path = "sim/joystick/yoke_heading_ratio", value_field = "yaw_input_ratio", valid_field = "yaw_input_valid" },
                { id = "cockpit", path = "sim/cockpit2/controls/yoke_heading_ratio", value_field = "yaw_input_cockpit_ratio", valid_field = "yaw_input_cockpit_valid" },
                { id = "flight_control", path = "sim/joystick/FC_hdng", value_field = "yaw_input_flight_control_ratio", valid_field = "yaw_input_flight_control_valid" }
            }
        }
    },
    arrays = {
        elevator = { path = "sim/flightmodel2/wing/elevator1_deg" },
        aileron = { path = "sim/flightmodel2/wing/aileron1_deg" },
        rudder = { path = "sim/flightmodel2/wing/rudder1_deg" }
    },
    surfaces = {
        elevator_1 = { array_key = "elevator", index = 8 },
        elevator_2 = { array_key = "elevator", index = 9 },
        aileron_1 = { array_key = "aileron", index = 0 },
        aileron_2 = { array_key = "aileron", index = 1 },
        rudder_1 = { array_key = "rudder", index = 10 },
        rudder_2 = { array_key = "rudder", index = 11 }
    },
    engines = {
        count = { path = "sim/aircraft/engine/acf_num_engines" },
        engine_type = { path = "sim/aircraft/prop/acf_en_type", array_kind = "int" },
        throttle = { path = "sim/cockpit2/engine/actuators/throttle_ratio", array_kind = "float" },
        throttle_jet_reverse = { path = "sim/cockpit2/engine/actuators/throttle_jet_rev_ratio", array_kind = "float" },
        throttle_beta_reverse = { path = "sim/cockpit2/engine/actuators/throttle_beta_rev_ratio", array_kind = "float" },
        -- cockpit2 is the generic display/indicator layer.  ENGN_N1_ remains a
        -- last-resort fallback because several study-level aircraft keep their
        -- displayed N1 in a custom engine model.
        n1_cockpit = { path = "sim/cockpit2/engine/indicators/N1_percent", array_kind = "float" },
        n1_flightmodel = { path = "sim/flightmodel/engine/ENGN_N1_", array_kind = "float" }
    },
    -- Aircraft-specific N1 providers are deliberately data-driven.  A provider
    -- is selected only when both the aircraft identity matches and its complete
    -- required DataRef set exists.  An invalid individual sample then falls back
    -- to cockpit2 and finally to the flight-model value without interrupting the
    -- landing recorder.
    n1 = {
        providers = {
            {
                id = "flightfactor_1sim",
                label_zh = "FlightFactor 仪表 N1",
                label_en = "FlightFactor indicated N1",
                mode = "scalar",
                identity_any = { "FLIGHTFACTOR", "BOEING757-FULL", "BOEING767-EXTENDED" },
                icao_any = { "B752", "B753", "B762", "B763", "B764", "B772", "B773", "B77L", "B77W", "B77F" },
                paths = { "1-sim/eng/N1/L", "1-sim/eng/N1/R" }
            },
            {
                id = "zibo_levelup_737ng",
                label_zh = "Zibo/LevelUp EICAS N1",
                label_en = "Zibo/LevelUp EICAS N1",
                mode = "scalar",
                identity_any = { "B737-800X", "738U_", "737NG SERIES", "ZIBO", "LEVELUP" },
                icao_any = { "B736", "B737", "B738", "B739", "B38M", "B39M" },
                paths = {
            ...
            {
                id = "inibuilds_a300",
                label_zh = "iniBuilds A300 仪表 N1",
                label_en = "iniBuilds A300 indicated N1",
                mode = "scalar",
                identity_any = { "A300 CARGO", "A300 PAX", "INIBUILDS A300", "INIBUILDS_A300" },
                icao_any = { "A306", "A30B" },
                paths = { "A300/engine/engine1_N1", "A300/engine/engine2_N1" }
            },
            {
                id = "rotate_md11",
                label_zh = "Rotate MD-11 仪表 N1",
                label_en = "Rotate MD-11 indicated N1",
                mode = "array",
                identity_any = { "ROTATE-MD-11", "ROTATE MD-11", "ROTATE_MD-11" },
                icao_any = { "MD11" },
                path = "Rotate/aircraft/systems/eng_n1"
            }
        },
        fallback_order = { "n1_cockpit", "n1_flightmodel" },
        fallback_labels = {
            n1_cockpit = {
                zh = "X-Plane cockpit2 仪表 N1",
                en = "X-Plane cockpit2 indicated N1"
            },
            n1_flightmodel = {
                zh = "X-Plane flightmodel N1 回退",
                en = "X-Plane flightmodel N1 fallback"
            }
        },
        selected = nil,
        used_sources = {}
    ...
    airbus = {
        active = false,
        family = false,
        lever = { path = "AirbusFBW/throttle_input" },
        mode = { path = "AirbusFBW/THRLeverMode" }
    },
    max_recorded_engines = 4,
    engine_count = 0,
    recorded_engine_count = 0,
    missing = {},
    engine_missing = {}
}

function LMM_CONTROL_REFS.read_magic_array(source, index)
    return source[index]
end

function LMM_CONTROL_REFS.initialize()
    for key, spec in pairs(LMM_CONTROL_REFS.inputs) do
        spec.available = false
        spec.selected_candidate = nil
        for i = 1, #spec.candidates do
            local candidate = spec.candidates[i]
            candidate.handle = XPLMFindDataRef(candidate.path)
            candidate.available = candidate.handle ~= nil
            if candidate.available then spec.available = true end
        end
        if not spec.available then
            LMM_CONTROL_REFS.missing[#LMM_CONTROL_REFS.missing + 1] = key .. " input candidates"
        end
    end
    for _, spec in pairs(LMM_CONTROL_REFS.arrays) do
        spec.handle = XPLMFindDataRef(spec.path)
        spec.available = false
        if spec.handle ~= nil and type(dataref_table) == "function" then
            local ok, array = pcall(dataref_table, spec.path)
            if ok and array ~= nil then
                spec.table = array
                spec.available = true
            end
        end
        if not spec.available then
            LMM_CONTROL_REFS.missing[#LMM_CONTROL_REFS.missing + 1] = spec.path
        end
    end
    for _, spec in pairs(LMM_CONTROL_REFS.engines) do
        spec.handle = XPLMFindDataRef(spec.path)
        spec.available = spec.handle ~= nil
        if spec.available and spec.array_kind ~= nil and type(dataref_table) == "function" then
            local ok, array = pcall(dataref_table, spec.path)
            if ok and array ~= nil then spec.table = array end
        end
        if not spec.available then
            LMM_CONTROL_REFS.engine_missing[#LMM_CONTROL_REFS.engine_missing + 1] = spec.path
        end
    end
    LMM_CONTROL_REFS.refresh_engine_context()
end

function LMM_CONTROL_REFS.refresh_engine_context()
    local count_spec = LMM_CONTROL_REFS.engines.count
    local count = count_spec.available and tonumber(XPLMGetDatai(count_spec.handle)) or 0
    if count == nil or count ~= count or count < 0 then count = 0 end
    LMM_CONTROL_REFS.engine_count = math.max(0, math.min(16, math.floor(count + 0.5)))
    LMM_CONTROL_REFS.recorded_engine_count = math.min(
        LMM_CONTROL_REFS.max_recorded_engines,
        LMM_CONTROL_REFS.engine_count
    )

    local aircraft_icao = string.upper(
        tostring(type(PLANE_ICAO) == "string" and PLANE_ICAO or "")
    )
    local identity = string.upper(
        tostring(type(AIRCRAFT_PATH) == "string" and AIRCRAFT_PATH or "")
            .. " "
            .. tostring(type(AIRCRAFT_FILENAME) == "string" and AIRCRAFT_FILENAME or "")
    )
    LMM_CONTROL_REFS.airbus.active = string.find(identity, "TOLISS", 1, true) ~= nil
    LMM_CONTROL_REFS.airbus.family = LMM_CONTROL_REFS.airbus.active
        or string.find(identity, "AIRBUS", 1, true) ~= nil
        or string.match(aircraft_icao, "^A3") ~= nil
        or aircraft_icao == "A20N"
        or aircraft_icao == "A21N"
        or aircraft_icao == "A318"
        or aircraft_icao == "A319"
        or aircraft_icao == "A320"
        or aircraft_icao == "A321"
    for _, spec in pairs({ LMM_CONTROL_REFS.airbus.lever, LMM_CONTROL_REFS.airbus.mode }) do
        spec.handle = LMM_CONTROL_REFS.airbus.active and XPLMFindDataRef(spec.path) or nil
        spec.available = spec.handle ~= nil
        spec.table = nil
        if spec == LMM_CONTROL_REFS.airbus.lever
            and spec.available
            and type(dataref_table) == "function" then
            local ok, array = pcall(dataref_table, spec.path)
            if ok and array ~= nil then spec.table = array end
        end
    end
    LMM_CONTROL_REFS.refresh_n1_provider(identity, aircraft_icao)
end

function LMM_CONTROL_REFS.read_float_array(spec, index, minimum, maximum)
    if spec == nil or not spec.available or spec.handle == nil then return 0, false end
    if spec.table ~= nil then
        local table_ok, table_value = pcall(LMM_CONTROL_REFS.read_magic_array, spec.table, index)
        table_value = table_ok and tonumber(table_value) or nil
        if table_value ~= nil and table_value == table_value
            and table_value >= minimum and table_value <= maximum then
            return table_value, true
        end
    end
    -- FlyWithLua/XPLM builds differ on whether returned array keys keep the
    -- original X-Plane index or are rebased to 0/1.  Read from element 0 up to
    -- the requested engine, then accept either 0-based or 1-based table keys.
    local ok, values = pcall(XPLMGetDatavf, spec.handle, 0, index + 1)
    local value = nil
    if ok and type(values) == "table" then
        value = tonumber(values[index]) or tonumber(values[index + 1])
    end
    if value == nil or value ~= value or value < minimum or value > maximum then
        return 0, false
    end
    return value, true
end

function LMM_CONTROL_REFS.read_int_array(spec, index, minimum, maximum)
    if spec == nil or not spec.available or spec.handle == nil then return 0, false end
    if spec.table ~= nil then
        local table_ok, table_value = pcall(LMM_CONTROL_REFS.read_magic_array, spec.table, index)
        table_value = table_ok and tonumber(table_value) or nil
        if table_value ~= nil and table_value == table_value
            and table_value >= minimum and table_value <= maximum then
            return math.floor(table_value + 0.5), true
        end
    end
    local ok, values = pcall(XPLMGetDatavi, spec.handle, 0, index + 1)
    local value = nil
    if ok and type(values) == "table" then
        value = tonumber(values[index]) or tonumber(values[index + 1])
    end
...
function LMM_CONTROL_REFS.identity_matches_any(text, candidates)
    if type(candidates) ~= "table" or #candidates == 0 then return false end
    for i = 1, #candidates do
        if string.find(text, candidates[i], 1, true) ~= nil then return true end
    end
    return false
end

function LMM_CONTROL_REFS.icao_matches_any(aircraft_icao, candidates)
    if type(candidates) ~= "table" or #candidates == 0 then return false end
    for i = 1, #candidates do
        if aircraft_icao == candidates[i] then return true end
    end
    return false
end

function LMM_CONTROL_REFS.n1_provider_matches(provider, identity, aircraft_icao)
    return LMM_CONTROL_REFS.identity_matches_any(identity, provider.identity_any)
        or LMM_CONTROL_REFS.icao_matches_any(aircraft_icao, provider.icao_any)
end

function LMM_CONTROL_REFS.bind_n1_array_spec(spec)
    spec.handle = XPLMFindDataRef(spec.path)
    spec.available = spec.handle ~= nil
    spec.table = nil
    if spec.available and type(dataref_table) == "function" then
        local ok, array = pcall(dataref_table, spec.path)
        if ok and array ~= nil then spec.table = array end
    end
...
function LMM_CONTROL_REFS.bind_n1_provider(provider)
    provider.available = false
    provider.handles = nil
    provider.spec = nil
    if provider.mode == "scalar" then
        provider.handles = {}
        local required = math.min(
            LMM_CONTROL_REFS.recorded_engine_count,
            type(provider.paths) == "table" and #provider.paths or 0
        )
        if required <= 0 then return false end
        for i = 1, #provider.paths do
            provider.handles[i] = XPLMFindDataRef(provider.paths[i])
        end
        for i = 1, required do
            if provider.handles[i] == nil then return false end
        end
        provider.available = true
        return true
    end
    if provider.mode == "array" and type(provider.path) == "string" then
        provider.spec = { path = provider.path, array_kind = "float" }
        provider.available = LMM_CONTROL_REFS.bind_n1_array_spec(provider.spec)
        return provider.available
    end
    return false
end

function LMM_CONTROL_REFS.refresh_n1_provider(identity, aircraft_icao)
    local n1 = LMM_CONTROL_REFS.n1
    n1.selected = nil
    n1.used_sources = {}
    for i = 1, #n1.providers do
        local provider = n1.providers[i]
        provider.available = false
        provider.handles = nil
        provider.spec = nil
        if LMM_CONTROL_REFS.n1_provider_matches(provider, identity, aircraft_icao)
            and LMM_CONTROL_REFS.bind_n1_provider(provider) then
            n1.selected = provider
            break
        end
    end
end

function LMM_CONTROL_REFS.read_float_scalar(handle, minimum, maximum)
    if handle == nil then return 0, false end
    local ok, raw_value = pcall(XPLMGetDataf, handle)
    local value = ok and tonumber(raw_value) or nil
    if value == nil or value ~= value or value < minimum or value > maximum then
        return 0, false
    end
    return value, true
...
function LMM_CONTROL_REFS.read_n1_provider(provider, index)
    if provider == nil or not provider.available then return 0, false end
    if provider.mode == "scalar" then
        return LMM_CONTROL_REFS.read_float_scalar(
            provider.handles and provider.handles[index + 1] or nil,
            0,
            200
        )
    end
    if provider.mode == "array" then
        return LMM_CONTROL_REFS.read_float_array(provider.spec, index, 0, 200)
    end
    return 0, false
end

function LMM_CONTROL_REFS.mark_n1_source(source_id)
    if source_id ~= nil and source_id ~= "" then
        LMM_CONTROL_REFS.n1.used_sources[source_id] = true
    end
end

function LMM_CONTROL_REFS.n1_source_description(source_id, is_english)
    local n1 = LMM_CONTROL_REFS.n1
    for i = 1, #n1.providers do
        local provider = n1.providers[i]
        if provider.id == source_id then
            local label = is_english and provider.label_en or provider.label_zh
            if provider.mode == "scalar" then
                return label .. " (" .. table.concat(provider.paths, ", ") .. ")"
            end
            return label .. " (" .. tostring(provider.path) .. ")"
        end
    end
    local fallback = n1.fallback_labels[source_id]
    local spec = LMM_CONTROL_REFS.engines[source_id]
    if fallback ~= nil and spec ~= nil then
        return (is_english and fallback.en or fallback.zh) .. " (" .. spec.path .. ")"
    end
    return tostring(source_id or "")
end

function LMM_CONTROL_REFS.n1_source_text(is_english)
    local n1 = LMM_CONTROL_REFS.n1
    local ordered = {}
    if n1.selected ~= nil and n1.used_sources[n1.selected.id] then
        ordered[#ordered + 1] = LMM_CONTROL_REFS.n1_source_description(n1.selected.id, is_english)
    end
    for i = 1, #n1.fallback_order do
        local source_id = n1.fallback_order[i]
        if n1.used_sources[source_id] then
            ordered[#ordered + 1] = LMM_CONTROL_REFS.n1_source_description(source_id, is_english)
        end
    end
    if #ordered > 0 then return table.concat(ordered, "; ") end
    if n1.selected ~= nil then
        return LMM_CONTROL_REFS.n1_source_description(n1.selected.id, is_english)
    end
    for i = 1, #n1.fallback_order do
        local source_id = n1.fallback_order[i]
        local spec = LMM_CONTROL_REFS.engines[source_id]
        if spec ~= nil and spec.available then
            return LMM_CONTROL_REFS.n1_source_description(source_id, is_english)
        end
    end
    return is_english and "Unavailable" or "不可用"
end

function LMM_CONTROL_REFS.read_throttle(index)
    if index < 0 or index >= LMM_CONTROL_REFS.recorded_engine_count then return 0, false end
    local engine_type, engine_type_valid = LMM_CONTROL_REFS.read_int_array(
        LMM_CONTROL_REFS.engines.engine_type,
        index,
        0,
        10
    )
    local forward, forward_valid = LMM_CONTROL_REFS.read_float_array(
        LMM_CONTROL_REFS.engines.throttle,
        index,
        -0.05,
        1.1
    )
    local jet, jet_valid = LMM_CONTROL_REFS.read_float_array(
        LMM_CONTROL_REFS.engines.throttle_jet_reverse,
        index,
        -1.1,
        1.1
    )
    local beta, beta_valid = LMM_CONTROL_REFS.read_float_array(
        LMM_CONTROL_REFS.engines.throttle_beta_reverse,
        index,
        -2.1,
        1.1
    )
    if engine_type_valid and (engine_type == 5 or engine_type == 7) and jet_valid then
        return math.max(-1, math.min(1, jet)), true
    end
    if engine_type_valid
        and (engine_type == 0 or engine_type == 1 or engine_type == 9 or engine_type == 10)
        and beta_valid then
        return math.max(-2, math.min(1, beta)), true
    end
    if not engine_type_valid and jet_valid and jet < -0.001 then return jet, true end
    if not engine_type_valid and beta_valid and beta < -0.001 then return beta, true end
    if forward_valid then return math.max(0, math.min(1, forward)), true end
    if jet_valid then return math.max(0, math.min(1, jet)), true end
    if beta_valid then return math.max(-2, math.min(1, beta)), true end
    return 0, false
end

function LMM_CONTROL_REFS.read_n1(index)
    if index < 0 or index >= LMM_CONTROL_REFS.recorded_engine_count then return 0, false end
    local n1 = LMM_CONTROL_REFS.n1
    if n1.selected ~= nil then
        local value, valid = LMM_CONTROL_REFS.read_n1_provider(n1.selected, index)
        if valid then
            LMM_CONTROL_REFS.mark_n1_source(n1.selected.id)
            return value, true
        end
    end
    for i = 1, #n1.fallback_order do
        local source_id = n1.fallback_order[i]
        local value, valid = LMM_CONTROL_REFS.read_float_array(
            LMM_CONTROL_REFS.engines[source_id],
            index,
            0,
            200
        )
        if valid then
            LMM_CONTROL_REFS.mark_n1_source(source_id)
            return value, true
        end
    end
    return 0, false
...
function LMM_CONTROL_REFS.read_airbus_lever(index)
    if not LMM_CONTROL_REFS.airbus.active
        or index < 0
        or index >= LMM_CONTROL_REFS.recorded_engine_count then
        return 0, false
    end
    return LMM_CONTROL_REFS.read_float_array(LMM_CONTROL_REFS.airbus.lever, index, -1.1, 1.1)
end

function LMM_CONTROL_REFS.airbus_detent(value, valid)
    if not valid then return "NA" end
    if value < -0.001 then return "REV" end
    if math.abs(value) <= 0.015 then return "IDLE" end
    if value >= 0.97 then return "TOGA" end
    if value >= 0.80 and value <= 0.93 then return "FLX/MCT" end
    if value >= 0.62 and value <= 0.75 then return "CL" end
    return "MANUAL"
end

function LMM_CONTROL_REFS.read_airbus_mode()
    local spec = LMM_CONTROL_REFS.airbus.mode
    if not LMM_CONTROL_REFS.airbus.active or not spec.available then return 0, false end
    local value = tonumber(XPLMGetDatai(spec.handle))
    if value == nil or value ~= value or value < -100 or value > 100 then return 0, false end
    if value >= 0 then return math.floor(value + 0.5), true end
    return math.ceil(value - 0.5), true
end

function LMM_CONTROL_REFS.read_input_candidate(candidate)
    if candidate == nil or not candidate.available or candidate.handle == nil then return 0, false end
    local value = tonumber(XPLMGetDataf(candidate.handle))
    if value == nil or value ~= value or math.abs(value) > 1.5 then return 0, false end
    return math.max(-1, math.min(1, value)), true
end

function LMM_CONTROL_REFS.capture_input_candidates(slot, key)
    local spec = LMM_CONTROL_REFS.inputs[key]
    if spec == nil then return end
    for i = 1, #spec.candidates do
        local candidate = spec.candidates[i]
        slot[candidate.value_field], slot[candidate.valid_field] =
            LMM_CONTROL_REFS.read_input_candidate(candidate)
    end
end

function LMM_CONTROL_REFS.input_candidate_is_active(candidate, slots, count)
    local minimum = nil
    local maximum = nil
    local maximum_absolute = 0
    local total_change = 0
    local previous = nil
    local valid_count = 0
    for i = 1, count do
        local slot = slots[i]
        if slot ~= nil and slot[candidate.valid_field] == true then
            local value = tonumber(slot[candidate.value_field])
            if value ~= nil and value == value then
                ...
                maximum_absolute = math.max(maximum_absolute, math.abs(value))
                if previous ~= nil then total_change = total_change + math.abs(value - previous) end
                previous = value
                valid_count = valid_count + 1
            end
        end
    end
    if valid_count == 0 then return false end
    local range = (maximum or 0) - (minimum or 0)
    return range >= 0.006 or total_change >= 0.02 or maximum_absolute >= 0.02
end
...
function LMM_CONTROL_REFS.input_source_text(is_english)
    local ids = {}
    local paths = {}
    for _, key in ipairs({ "pitch", "roll", "yaw" }) do
        local selected = LMM_CONTROL_REFS.inputs[key].selected_candidate
        if selected ~= nil then
            ids[selected.id] = true
            paths[#paths + 1] = string.upper(string.sub(key, 1, 1))
                .. string.sub(key, 2) .. "=" .. selected.path
        end
    end
    local id_count = 0
    local only_id = nil
    ...
    if id_count == 1 and only_id == "joystick" then
        return is_english and "standard joystick-ratio DataRefs" or "标准摇杆比例 DataRef"
    end
    if id_count == 1 and only_id == "cockpit" then
        return is_english and "cockpit2 cockpit-control input fallback"
            or "cockpit2 驾驶舱操纵输入回退接口"
    end
    if id_count == 1 and only_id == "flight_control" then
        return is_english and "FC flight-control command fallback (includes artificial stability)"
            or "FC 飞控指令回退接口（含人工增稳）"
    end
    return #paths > 0 and table.concat(paths, "; ")
        or (is_english and "Unavailable" or "不可用")
end

function LMM_CONTROL_REFS.read_surface(key)
    local spec = LMM_CONTROL_REFS.surfaces[key]
    if spec == nil then return 0, false end
    local array_spec = LMM_CONTROL_REFS.arrays[spec.array_key]
    if array_spec == nil or not array_spec.available then return 0, false end
    local ok, value = pcall(
        LMM_CONTROL_REFS.read_magic_array,
        array_spec.table,
        spec.index
    )
    value = ok and tonumber(value) or nil
    if value == nil or value ~= value or math.abs(value) > 180 then return 0, false end
    return value, true
end

function LMM_CONTROL_REFS.capture(slot, shared_inputs)
    if not shared_inputs then
        LMM_CONTROL_REFS.capture_input_candidates(slot, "pitch")
        LMM_CONTROL_REFS.capture_input_candidates(slot, "roll")
        LMM_CONTROL_REFS.capture_input_candidates(slot, "yaw")
    end
    slot.elevator_1_deg, slot.elevator_1_valid = LMM_CONTROL_REFS.read_surface("elevator_1")
    slot.elevator_2_deg, slot.elevator_2_valid = LMM_CONTROL_REFS.read_surface("elevator_2")
    slot.aileron_1_deg, slot.aileron_1_valid = LMM_CONTROL_REFS.read_surface("aileron_1")
    slot.aileron_2_deg, slot.aileron_2_valid = LMM_CONTROL_REFS.read_surface("aileron_2")
    slot.rudder_1_deg, slot.rudder_1_valid = LMM_CONTROL_REFS.read_surface("rudder_1")
    slot.rudder_2_deg, slot.rudder_2_valid = LMM_CONTROL_REFS.read_surface("rudder_2")

    slot.throttle_1_ratio, slot.throttle_1_valid = LMM_CONTROL_REFS.read_throttle(0)
    slot.throttle_2_ratio, slot.throttle_2_valid = LMM_CONTROL_REFS.read_throttle(1)
    slot.throttle_3_ratio, slot.throttle_3_valid = LMM_CONTROL_REFS.read_throttle(2)
    slot.throttle_4_ratio, slot.throttle_4_valid = LMM_CONTROL_REFS.read_throttle(3)
    slot.n1_1_percent, slot.n1_1_valid = LMM_CONTROL_REFS.read_n1(0)
    slot.n1_2_percent, slot.n1_2_valid = LMM_CONTROL_REFS.read_n1(1)
    slot.n1_3_percent, slot.n1_3_valid = LMM_CONTROL_REFS.read_n1(2)
    slot.n1_4_percent, slot.n1_4_valid = LMM_CONTROL_REFS.read_n1(3)

    slot.airbus_lever_1_ratio, slot.airbus_lever_1_valid = LMM_CONTROL_REFS.read_airbus_lever(0)
    slot.airbus_lever_2_ratio, slot.airbus_lever_2_valid = LMM_CONTROL_REFS.read_airbus_lever(1)
    slot.airbus_lever_3_ratio, slot.airbus_lever_3_valid = LMM_CONTROL_REFS.read_airbus_lever(2)
    slot.airbus_lever_4_ratio, slot.airbus_lever_4_valid = LMM_CONTROL_REFS.read_airbus_lever(3)
    slot.airbus_detent_1 = LMM_CONTROL_REFS.airbus_detent(
        slot.airbus_lever_1_ratio,
        slot.airbus_lever_1_valid
    )
    slot.airbus_detent_2 = LMM_CONTROL_REFS.airbus_detent(
        slot.airbus_lever_2_ratio,
        slot.airbus_lever_2_valid
    )
    slot.airbus_detent_3 = LMM_CONTROL_REFS.airbus_detent(
        slot.airbus_lever_3_ratio,
        slot.airbus_lever_3_valid
    )
    slot.airbus_detent_4 = LMM_CONTROL_REFS.airbus_detent(
        slot.airbus_lever_4_ratio,
        slot.airbus_lever_4_valid
    )
    slot.airbus_mode, slot.airbus_mode_valid = LMM_CONTROL_REFS.read_airbus_mode()
    if LMM_CONTROL_REFS.observer then LMM_CONTROL_REFS.observer:capture(slot) end
end

function LMM_CONTROL_REFS.available_count()
    local count = 0
    for _, spec in pairs(LMM_CONTROL_REFS.inputs) do
        if spec.available then count = count + 1 end
    end
    for _, spec in pairs(LMM_CONTROL_REFS.surfaces) do
        local array_spec = LMM_CONTROL_REFS.arrays[spec.array_key]
        if array_spec ~= nil and array_spec.available then count = count + 1 end
    end
    return count
end

LMM_CONTROL_REFS.initialize()

if #lmm_missing_datarefs > 0 and logMsg then
    logMsg("[StarLux LMM] Missing required DataRefs: " .. table.concat(lmm_missing_datarefs, ", "))
end

if #LMM_CONTROL_REFS.missing > 0 and logMsg then
    logMsg("[StarLux LMM] Optional control DataRefs unavailable: "
        .. table.concat(LMM_CONTROL_REFS.missing, ", "))
end
...
local flare_trace = {
    ...
    post_touch_active = false,
    post_touch_deadline = 0,
    ground_stable_since = 0,
    last_on_ground = 0,
    airborne_episode_count = 0,
    touch_count = 0,
    end_reason = "等待接地",
    touch_sample_index = 0,
    touch_bucket_index = 0,
    next_sample_time = 0,
...
for i = 1, FLARE_CONFIG.max_samples do
    flare_trace.slots[i] = {
        ...
        roll_input_ratio = 0,
        yaw_input_ratio = 0,
        pitch_input_cockpit_ratio = 0,
        roll_input_cockpit_ratio = 0,
        yaw_input_cockpit_ratio = 0,
        pitch_input_flight_control_ratio = 0,
        roll_input_flight_control_ratio = 0,
        yaw_input_flight_control_ratio = 0,
        elevator_1_deg = 0,
        elevator_2_deg = 0,
        aileron_1_deg = 0,
        aileron_2_deg = 0,
        ...
        n1_3_percent = 0,
        n1_4_percent = 0,
        airbus_lever_1_ratio = 0,
        airbus_lever_2_ratio = 0,
        airbus_lever_3_ratio = 0,
        airbus_lever_4_ratio = 0,
        airbus_detent_1 = "NA",
        airbus_detent_2 = "NA",
        airbus_detent_3 = "NA",
        airbus_detent_4 = "NA",
        airbus_mode = 0,
        pitch_input_valid = false,
        roll_input_valid = false,
        yaw_input_valid = false,
        pitch_input_cockpit_valid = false,
        roll_input_cockpit_valid = false,
        yaw_input_cockpit_valid = false,
        pitch_input_flight_control_valid = false,
        roll_input_flight_control_valid = false,
        yaw_input_flight_control_valid = false,
        elevator_1_valid = false,
        elevator_2_valid = false,
        aileron_1_valid = false,
        aileron_2_valid = false,
        rudder_1_valid = false,
        rudder_2_valid = false,
        throttle_1_valid = false,
        throttle_2_valid = false,
        throttle_3_valid = false,
        throttle_4_valid = false,
        n1_1_valid = false,
        n1_2_valid = false,
        n1_3_valid = false,
        n1_4_valid = false,
        airbus_lever_1_valid = false,
        airbus_lever_2_valid = false,
        airbus_lever_3_valid = false,
        airbus_lever_4_valid = false,
        airbus_mode_valid = false
    }
end
...
log_tools.popup_style = {
    ...
    colors = {
        NICE = { 0.07, 0.32, 0.62 },
        STABLE = { 0.08, 0.50, 0.24 },
        ATTENTION = { 0.78, 0.52, 0.06 },
        UNSTABLE = { 0.72, 0.10, 0.12 }
    },
    defaults = {
        NICE = { 0.07, 0.32, 0.62 },
        STABLE = { 0.08, 0.50, 0.24 },
...
log_tools.runway_config = {
    ...
    -- v2 按 apt.dat 数据源分别保存机场偏移；地景变化时只重建变化分片。
    index_version = 2,
    index_chunk_bytes = 65536,
    index_max_chunks_per_step = 4,
    index_min_step_interval_seconds = 0.05,
    index_checkpoint_seconds = 30.0,
    -- 保存采用批量字符串写入；扫描预算仍保守，缓存落盘无需再持续数分钟。
    index_save_lines_per_step = 2048,
    index_save_lines_per_chunk = 512,
    index_ground_max_speed_kt = 5.0,
    index_cruise_min_agl_ft = 6000.0,
    index_cruise_max_vertical_speed_mps = 3.0,
    index_complete_notice_seconds = 12.0,
    index_debug_hold_seconds = 3.0,
    runway_debug_hold_seconds = 5.0,
...
log_tools.runway_state = {
    ...
    index = {
        path = join_path(LMM_BASE_DIRECTORY, "LMM_Apt_Index_v1.cache"),
        temp_path = join_path(LMM_BASE_DIRECTORY, ".LMM_Apt_Index_v1.tmp"),
        backup_path = join_path(LMM_BASE_DIRECTORY, ".LMM_Apt_Index_v1.bak"),
        entries = {},
        entry_count = 0,
        source_entries = {},
        source_complete = {},
        source_resume_offsets = {},
        build_required = {},
        ...
        complete = false,
        dirty = false,
        notice_build_required = false,
        notice_completion_pending = false,
        notice_complete_until = 0,
        fps_ema = 0,
        raw_fps = 0,
        adaptive_tier = "NORMAL",
        adaptive_chunks = 1,
        adaptive_interval = 0.15,
        last_update_now = 0,
        debug_activity_until = 0,
        debug_last_bytes = 0,
        build_file = nil,
        build_source_index = 1,
        build_resume_offset = 0,
        build_partial = "",
        build_partial_offset = 0,
        last_step_at = -1000,
...
local landing_jobs = {
    ...
    runway_deadline = 0,
    log_write = nil,
    -- 放在既有状态表中，避免大型 Lua 5.1 主代码块新增顶层局部变量。
    write_chunk_bytes = 8192
}
...
-- 全局语言：界面、弹窗、数据库提示、TXT 和插件启动的阅读器共用。
function log_tools.settings_text(chinese, english)
    -- The pre-12.4.4 ImGui atlas cannot guarantee CJK glyphs. Report and popup
    -- language remain global; only the explicit legacy edition uses ASCII UI.
    if log_tools.native_ui.force_compatibility then return english end
    return log_tools.ui_text(chinese, english)
end

function log_tools.popup_language()
    return runtime_state.document_language
end

function log_tools.legacy_font_size()
    -- Never change language to work around missing glyphs. Compatibility mode
    -- uses the standard Unicode font for Chinese, preserving the saved size.
    return runtime_state.document_language == "zh" and "normal" or log_tools.popup_style.font_size
end

function log_tools.popup_text(chinese, english)
    if log_tools.popup_language() == "en" then return english end
    return chinese
end

function log_tools.clamp_color(value)
    return math.max(0, math.min(1, tonumber(value) or 0))
end

function log_tools.rgb_setting_text(color)
    return string.format(
        "%d,%d,%d",
        math.floor(log_tools.clamp_color(color[1]) * 255 + 0.5),
        math.floor(log_tools.clamp_color(color[2]) * 255 + 0.5),
        math.floor(log_tools.clamp_color(color[3]) * 255 + 0.5)
    )
end

function log_tools.parse_rgb_setting(value, color)
    local red, green, blue = tostring(value or ""):match(
        "^%s*([%+%-]?[%d%.]+)%s*,%s*([%+%-]?[%d%.]+)%s*,%s*([%+%-]?[%d%.]+)%s*$"
    )
    if red == nil or green == nil or blue == nil then return false end
    red = tonumber(red)
    ...
    color[1] = log_tools.clamp_color(red)
    color[2] = log_tools.clamp_color(green)
    color[3] = log_tools.clamp_color(blue)
    return true
end

function log_tools.reset_report_notice()
    landing_report_notice.text = ""
    landing_report_notice.file_name = ""
    landing_report_notice.path = ""
    landing_report_notice.ready = false
    landing_report_notice.flash_started_at = 0
    landing_report_notice.flash_until_time = 0
    landing_report_notice.debug_until_time = 0
end

function log_tools.start_report_flash(now)
    if landing_report_notice.ready == false
        or landing_report_notice.flash_started_at > 0
        or POPUP_MODE == "off"
        or POPUP_MODE == "clean"
        or runtime_state.replay_active then
        return false
    end
    landing_report_notice.flash_started_at = now
    landing_report_notice.flash_until_time = now + landing_report_notice.pulse_seconds * 6
    return true
end

function log_tools.report_flash_state(now)
    if landing_report_notice.flash_started_at <= 0
        or now >= landing_report_notice.flash_until_time then
        return false, false
    end
    local phase = math.floor(
        (now - landing_report_notice.flash_started_at) / landing_report_notice.pulse_seconds
    )
    return phase >= 0 and phase < 6, phase >= 0 and phase < 6 and phase % 2 == 0
end

function log_tools.popup_text_color(red, green, blue)
    if log_tools.popup_style.text_color_mode == "white" then return 1, 1, 1 end
    if log_tools.popup_style.text_color_mode == "black" then return 0, 0, 0 end
    -- 自动高对比反色：亮色底板使用黑字，暗色底板使用白字。
    local luminance = red * 0.299 + green * 0.587 + blue * 0.114
    if luminance >= 0.52 then return 0, 0, 0 end
    return 1, 1, 1
end

function log_tools.imgui_rgba(red, green, blue, alpha)
    local r = math.floor(log_tools.clamp_color(red) * 255 + 0.5)
    local g = math.floor(log_tools.clamp_color(green) * 255 + 0.5)
    local b = math.floor(log_tools.clamp_color(blue) * 255 + 0.5)
    local a = math.floor(log_tools.clamp_color(alpha) * 255 + 0.5)
    return a * 16777216 + b * 65536 + g * 256 + r
end

function log_tools.popup_font_metrics(layout)
    local font_size = log_tools.legacy_font_size()
    if layout == "vertical" then
        if font_size == "large" then return 18, 28, 16, 30, 280 end
        if font_size == "small" then return 10, 21, 14, 26, 235 end
        return 12, 27, 18, 31, 265
    end
    if font_size == "large" then return 18, 24, 14, 16, 360 end
    if font_size == "small" then return 10, 15, 12, 16, 330 end
    return 12, 18, 18, 14, 395
end

function log_tools.popup_measure_text(value)
    local ok, width
    local font_size = log_tools.legacy_font_size()
    if font_size == "large" then
        ok, width = pcall(measure_string, tostring(value or ""), "Helvetica_18")
    elseif font_size == "small" then
        ok, width = pcall(measure_string, tostring(value or ""), "Helvetica_10")
    else
        ok, width = pcall(measure_string, tostring(value or ""))
    end
    if ok and type(width) == "number" and width >= 0 then return width end
    local approximate_size = font_size == "large" and 18
        or (font_size == "small" and 10 or 12)
    return string.len(tostring(value or "")) * approximate_size * 0.58
end

function log_tools.draw_popup_glyphs(x, y, value)
    if log_tools.legacy_font_size() == "large" then
        draw_string_Helvetica_18(x, y, value)
    elseif log_tools.legacy_font_size() == "small" then
        draw_string_Helvetica_10(x, y, value)
    else
        draw_string(x, y, value)
    end
end

function log_tools.draw_popup_text(x, y, value)
    log_tools.draw_popup_glyphs(x, y, value)
    if log_tools.popup_style.font_weight == "medium" then
        log_tools.draw_popup_glyphs(x + 1, y, value)
    elseif log_tools.popup_style.font_weight == "bold" then
        log_tools.draw_popup_glyphs(x + 1, y, value)
        log_tools.draw_popup_glyphs(x, y + 1, value)
    end
end

function log_tools.draw_database_status(screen_w, screen_h, now)
    local state = log_tools.runway_state
    local index = state.index
    local lines = {}
    local completion_visible = index.notice_complete_until > now
    local initialization_visible = runtime_state.runway_detection_enabled == true
        and #state.sources > 0
        and (
            (index.notice_build_required == true and index.complete ~= true)
            or index.notice_completion_pending == true
        )

    if completion_visible then
        lines[#lines + 1] = {
            text = log_tools.ui_text(
                index.incremental_update == true
                    and "Starlux_LMM数据库增量更新完成 100%，祝你飞行愉快"
                    or "Starlux_LMM数据库建立完成 100%，祝你飞行愉快",
                index.incremental_update == true
                    and "Starlux_LMM database update complete 100%. Have a pleasant flight."
                    or "Starlux_LMM database ready 100%. Have a pleasant flight."
            ),
            red = 0.50,
            green = 0.96,
            blue = 0.68
        }
    elseif initialization_visible then
        lines[#lines + 1] = {
            text = string.format(
                log_tools.ui_text(
                    index.incremental_update == true
                        and "Starlux_LMM数据库增量更新中 %d%%"
                        or "Starlux_LMM数据库初始化中 %d%%",
                    index.incremental_update == true
                        and "Starlux_LMM database updating %d%%"
                        or "Starlux_LMM database initializing %d%%"
                ),
                log_tools.apt_index_progress_percent()
            ),
            red = 0.78,
            green = 0.88,
            blue = 0.96
    ...
    if DEBUG_MODE == true then
        local show_database_debug = initialization_visible
            or completion_visible
            or index.save_active == true
            or index.debug_activity_until > now
        if show_database_debug then
            local database_status = "PAUSED"
            if index.last_status == "save_error" then
                database_status = "SAVE_ERROR"
            elseif index.last_status == "build_error" then
                database_status = "BUILD_ERROR"
            elseif index.save_active then
                database_status = "SAVING"
            elseif index.notice_completion_pending then
                database_status = "SAVE_PENDING"
            elseif index.complete then
                database_status = "READY"
            elseif index.debug_activity_until > now then
                database_status = "SCANNING"
            end
            lines[#lines + 1] = {
                text = string.format(
                    "DB_SCAN status=%s progress=%d%% fps=%.1f tier=%s budget=%dx64KiB/%.3fs airports=%d sources=%d reused/%d pending",
                    database_status,
                    log_tools.apt_index_progress_percent(),
                    tonumber(index.fps_ema) or 0,
                    tostring(index.adaptive_tier or "NORMAL"),
                    tonumber(index.adaptive_chunks) or 1,
                    tonumber(index.adaptive_interval) or 0.15,
                    tonumber(index.entry_count) or 0,
                    tonumber(index.reused_source_count) or 0,
                    tonumber(index.pending_source_count) or 0
                ),
                red = 0.68,
                green = 0.82,
                blue = 0.90
            }
            local source = state.sources[index.build_source_index or 0]
            if source ~= nil then
                lines[#lines + 1] = {
                    text = string.format(
                        "DB_SOURCE %d/%d offset=%.1f/%.1fMiB %s",
                        tonumber(index.build_source_index) or 0,
                        #state.sources,
                        (tonumber(index.build_resume_offset) or 0) / 1048576,
                        (tonumber(source.size) or 0) / 1048576,
                        tostring(source.label or "apt.dat")
                    ),
                    red = 0.62,
                    green = 0.76,
                    blue = 0.84
            ...
            if tostring(index.last_reason or "") ~= "" then
                lines[#lines + 1] = {
                    text = "DB_REASON " .. tostring(index.last_reason),
                    red = 0.82,
                    green = 0.72,
                    blue = 0.48
                }
        ...
        local scan_debug = state.scan_debug
        if scan_debug.visible_until > now then
            lines[#lines + 1] = {
                text = string.format(
                    "APT_SCAN target=%s mode=%s status=%s route=%s",
                    tostring(scan_debug.target_airport or ""),
                    tostring(scan_debug.mode or "idle"),
                    tostring(scan_debug.status or "idle"),
                    tostring(scan_debug.route or "NONE")
                ),
                red = 0.90,
                green = 0.78,
                blue = 0.50
            ...
            lines[#lines + 1] = {
                text = string.format(
                    "APT_SOURCE %d/%d offset=%.1fMiB %s",
                    tonumber(scan_debug.source_index) or 0,
                    tonumber(scan_debug.source_count) or 0,
                    (tonumber(scan_debug.byte_offset) or 0) / 1048576,
                    tostring(scan_debug.source_label or "apt.dat")
                ),
                red = 0.80,
                green = 0.70,
                blue = 0.46
            }
            if tostring(scan_debug.reason or "") ~= "" then
                lines[#lines + 1] = {
                    text = "APT_REASON " .. tostring(scan_debug.reason),
                    red = 0.92,
                    green = 0.56,
                    blue = 0.45
                }
    ...
    for i = 1, #lines do
        local measure_ok, measured = pcall(measure_string, lines[i].text)
        if measure_ok and tonumber(measured) ~= nil then
            maximum_width = math.max(maximum_width, measured)
        else
            maximum_width = math.max(maximum_width, #lines[i].text * 6.5)
        end
    end
    local panel_width = math.min(math.max(260, maximum_width + 22), math.max(260, screen_w - 16))
    local panel_height = 12 + #lines * 16
    local panel_x = 8
    local panel_top = screen_h - 8
    local panel_y = panel_top - panel_height

    XPLMSetGraphicsState(0, 0, 0, 1, 1, 0, 0)
    glColor4f(0.015, 0.025, 0.035, DEBUG_MODE and 0.62 or 0.42)
    glRectf(panel_x, panel_y, panel_x + panel_width, panel_top)
    glColor4f(0.25, 0.58, 0.72, 0.72)
    glRectf(panel_x, panel_y, panel_x + 3, panel_top)
    for i = 1, #lines do
        local item = lines[i]
        glColor4f(item.red, item.green, item.blue, 0.96)
        draw_string(panel_x + 10, panel_top - 14 - (i - 1) * 16, item.text)
    end
end

function log_tools.popup_centerline_text()
    if runtime_state.runway_detection_enabled == false then
        return log_tools.popup_text("中心线偏差：未启用", "Centerline deviation: disabled")
    end
    if landing_context.runway_detected then
        local side = log_tools.popup_text("居中", "centred")
        if landing_context.centerline_side == "LEFT" then
            side = log_tools.popup_text("左侧", "left")
        elseif landing_context.centerline_side == "RIGHT" then
            side = log_tools.popup_text("右侧", "right")
        end
        return string.format(
            log_tools.popup_text("中心线偏差 %.1f 米（%s）", "Centerline deviation %.1f m (%s)"),
            landing_context.centerline_offset_m,
            side
        )
    end
    if landing_jobs.context_pending
        or landing_context.runway_status == "pending"
        or log_tools.runway_state.active then
        return log_tools.popup_text("中心线偏差：计算中", "Centerline deviation: calculating")
    end
    return log_tools.popup_text("中心线偏差：无法计算", "Centerline deviation: unavailable")
end
...
local function begin_landing_context()
    landing_context.recording_id=log_tools.recording.id
    local aircraft_icao = ""
...
function log_tools.apt_source_signature(file, source_size)
    local size = math.max(0, tonumber(source_size) or 0)
    local sample_bytes = 1024
    local positions = {
        0,
    ...
    for i = 1, #positions do
        local position = math.min(math.max(0, positions[i]), math.max(0, size - 1))
        if file:seek("set", position) ~= nil then
            local sample = file:read(math.min(sample_bytes, math.max(0, size - position))) or ""
            hash = (hash * 33 + position % 251) % 2147483647
            for j = 1, #sample do
                hash = (hash * 33 + string.byte(sample, j)) % 2147483647
            end
        end
    end
...
function log_tools.add_apt_source(path, label)
    ...
    local source_size = probe:seek("end") or 0
    local source_signature = log_tools.apt_source_signature(probe, source_size)
    probe:close()
...
function log_tools.discover_apt_sources()
    local state = log_tools.runway_state
    if log_tools.close_apt_index_io ~= nil then log_tools.close_apt_index_io() end
    state.sources = {}
    state.cache = {}
    state.airport_nav = {}
    ...
    if root == "" then
        ...
        if log_tools.reset_apt_index ~= nil then log_tools.reset_apt_index("X-Plane root unavailable") end
        return 0
    end
    ...
    if log_tools.load_apt_index ~= nil then log_tools.load_apt_index() end
    return #state.sources
end

-- apt.dat 索引使用独立缓存按数据源保存“机场标识 -> 字节偏移”。
-- 启动时再按 scenery_packs.ini 当前优先级合并；新增、删除、排序或单个地景更新
-- 都不再让全部已验证数据源失效。
-- 所有函数继续挂在 log_tools 上，避免增加 Lua 5.1 主代码块的顶层局部函数数量。
function log_tools.apt_index_hex_encode(value)
    return (tostring(value or ""):gsub(".", function(char)
        return string.format("%02X", string.byte(char))
    end))
end

function log_tools.apt_index_hex_decode(value)
    local encoded = tostring(value or "")
    if encoded:find("[^0-9A-Fa-f]") ~= nil or #encoded % 2 ~= 0 then return nil end
    return (encoded:gsub("%x%x", function(pair)
        return string.char(tonumber(pair, 16))
    end))
end

function log_tools.apt_index_source_key(path)
    local key = log_tools.normalize_xplane_path(path)
    if PATH_SEPARATOR == "\\" then key = string.lower(key) end
    return key
end

function log_tools.rebuild_apt_index_entries()
    local state = log_tools.runway_state
    local index = state.index
    index.entries = {}
    index.entry_count = 0
    for source_index = 1, #state.sources do
        if index.source_complete[source_index] == true then
            local source_entries = index.source_entries[source_index] or {}
            for airport_id, offset in pairs(source_entries) do
                if index.entries[airport_id] == nil then
                    index.entries[airport_id] = {
...
function log_tools.merge_apt_index_source(source_index)
    local index = log_tools.runway_state.index
    local source_entries = index.source_entries[source_index] or {}
    for airport_id, offset in pairs(source_entries) do
        local existing = index.entries[airport_id]
        if existing == nil then
            index.entries[airport_id] = {
                source_index = source_index,
                offset = offset
            }
            index.entry_count = (index.entry_count or 0) + 1
        elseif source_index < (existing.source_index or 2147483647) then
...
function log_tools.find_next_apt_index_source(start_index)
    local state = log_tools.runway_state
    local index = state.index
    for i = math.max(1, tonumber(start_index) or 1), #state.sources do
        if index.source_complete[i] ~= true then return i end
    end
    return #state.sources + 1
end

function log_tools.refresh_apt_index_pending_count()
    local state = log_tools.runway_state
    local index = state.index
    local pending = 0
    for i = 1, #state.sources do
        if index.source_complete[i] ~= true then pending = pending + 1 end
    end
    index.pending_source_count = pending
    return pending
end

function log_tools.apt_index_progress_percent()
    local state = log_tools.runway_state
    local index = state.index
    if index.complete == true then return 100 end
    local total_bytes = 0
    local completed_bytes = 0
    for i = 1, #state.sources do
        if index.build_required[i] == true then
            local source_size = math.max(0, tonumber(state.sources[i].size) or 0)
            local source_weight = math.max(1, source_size)
            total_bytes = total_bytes + source_weight
            if index.source_complete[i] == true then
                completed_bytes = completed_bytes + source_weight
            elseif source_size > 0 then
                local resume_offset = tonumber(index.source_resume_offsets[i]) or 0
                if i == index.build_source_index then
                    resume_offset = math.max(resume_offset, tonumber(index.build_resume_offset) or 0)
                end
                completed_bytes = completed_bytes + math.min(source_size, math.max(0, resume_offset))
            end
        end
    end
    if total_bytes <= 0 then return 0 end
    -- 尚未完成时最高显示 99%，避免缓存仍在扫描却提前出现 100%。
    return math.max(0, math.min(99, math.floor(completed_bytes * 100 / total_bytes)))
end

function log_tools.apt_index_entry_is_trusted(entry)
    local index = log_tools.runway_state.index
    local source_index = tonumber(entry and entry.source_index) or 0
    if source_index < 1 or index.source_complete[source_index] ~= true then return false end
    -- 未完成的高优先级分片可能覆盖当前机场；此时必须继续使用顺序扫描回退。
    for i = 1, source_index - 1 do
        if index.source_complete[i] ~= true then return false end
    end
    return true
end

function log_tools.pause_apt_index_build()
    local index = log_tools.runway_state.index
    if index.build_file ~= nil then pcall(function() index.build_file:close() end) end
    index.build_file = nil
    -- resume_offset 始终指向尚未处理的第一个字节；丢弃 partial 后可安全重读。
    index.build_partial = ""
    index.build_partial_offset = index.build_resume_offset or 0
    if (index.build_source_index or 0) >= 1 then
        index.source_resume_offsets[index.build_source_index] = index.build_resume_offset or 0
    end
end

function log_tools.close_apt_index_io()
    local index = log_tools.runway_state.index
    log_tools.pause_apt_index_build()
    if index.save_file ~= nil then pcall(function() index.save_file:close() end) end
    index.save_file = nil
    index.save_active = false
    index.save_phase = "idle"
    index.save_source_index = 1
    index.save_entry_key = nil
end

function log_tools.reset_apt_index(reason)
    local index = log_tools.runway_state.index
    log_tools.close_apt_index_io()
    index.entries = {}
    index.entry_count = 0
    index.source_entries = {}
    index.source_complete = {}
    index.source_resume_offsets = {}
    index.build_required = {}
    for i = 1, #log_tools.runway_state.sources do index.build_required[i] = true end
    index.reused_source_count = 0
    index.pending_source_count = #log_tools.runway_state.sources
    index.incremental_update = false
    index.loaded = true
    index.complete = false
    index.dirty = false
    index.notice_build_required = #log_tools.runway_state.sources > 0
    index.notice_completion_pending = false
    index.notice_complete_until = 0
    index.build_source_index = 1
    index.build_resume_offset = 0
    index.build_partial = ""
    index.build_partial_offset = 0
    index.last_step_at = -1000
    index.last_checkpoint_at = 0
    index.save_requested = false
    index.save_retry_after = 0
    index.last_status = #log_tools.runway_state.sources > 0 and "building" or "unavailable"
    index.last_reason = tostring(reason or "")
end

function log_tools.load_apt_index()
    local state = log_tools.runway_state
    local index = state.index
    log_tools.reset_apt_index("索引尚未建立")
    if #state.sources == 0 then return false end

    local file = io.open(index.path, "rb")
    if file == nil then
        -- 上次退出若恰好发生在原子轮换窗口，优先恢复仍完整的上一版缓存。
        local backup = io.open(index.backup_path, "rb")
        if backup ~= nil then
            backup:close()
            if os.rename(index.backup_path, index.path) ~= nil then
                file = io.open(index.path, "rb")
            end
        end
    end
    if file == nil then return false end
    local header = file:read("*l")
    if header ~= "LMM_APT_INDEX\t" .. tostring(log_tools.runway_config.index_version) then
        file:close()
        index.last_reason = "索引版本不匹配"
        return false
    end

    local saved_sources = {}
    local saved_source_count = 0
    local saved_complete = nil
    local saved_source_index = nil
    local saved_resume_offset = nil
    local saw_end = false
    ...
    for line in file:lines() do
        ...
        local source_index, source_size, source_signature, source_complete, encoded_path, encoded_label =
            line:match("^SOURCE\t(%d+)\t(%d+)\t(%d+)\t([01])\t([0-9A-Fa-f]*)\t([0-9A-Fa-f]*)$")
        if source_index ~= nil then
            local decoded_path = log_tools.apt_index_hex_decode(encoded_path)
            local decoded_label = log_tools.apt_index_hex_decode(encoded_label)
            local numeric_source_index = tonumber(source_index)
            if saved_sources[numeric_source_index] ~= nil then parse_error = true end
            saved_sources[numeric_source_index] = {
                saved_index = numeric_source_index,
                size = tonumber(source_size),
                signature = tonumber(source_signature),
                path = decoded_path,
                label = decoded_label,
                complete = source_complete == "1",
                entries = {},
                invalid = decoded_path == nil or decoded_label == nil
            }
            saved_source_count = saved_source_count + 1
        else
            local complete_flag, progress_source, progress_offset =
                line:match("^PROGRESS\t([01])\t(%d+)\t(%d+)$")
            if complete_flag ~= nil then
                saved_complete = complete_flag == "1"
                saved_source_index = tonumber(progress_source)
                saved_resume_offset = tonumber(progress_offset)
            else
                local airport_source, airport_id, airport_offset =
                    line:match("^SOURCE_AIRPORT\t(%d+)\t([^\t]+)\t(%d+)$")
                if airport_id ~= nil then
                    local saved = saved_sources[tonumber(airport_source)]
                    local normalized_airport = string.upper(trim_text(airport_id))
                    local numeric_offset = tonumber(airport_offset)
                    if saved == nil or normalized_airport == "" or numeric_offset == nil then
                        parse_error = true
                    elseif numeric_offset < 0 or numeric_offset >= (saved.size or 0) then
                        -- 单个分片条目损坏时只让该数据源失效，其余缓存仍可复用。
                        saved.invalid = true
                    elseif saved.entries[normalized_airport] == nil then
                        saved.entries[normalized_airport] = numeric_offset
                    end
                else
                    parse_error = true
                end
    ...
    if parse_error
        or not saw_end
        or saved_complete == nil
        or saved_source_index == nil
        or saved_resume_offset == nil then
        index.last_reason = "索引文件不完整"
        return false
    end
    local saved_by_path = {}
    for _, saved in pairs(saved_sources) do
        if saved.invalid ~= true then
            local source_key = log_tools.apt_index_source_key(saved.path)
            if source_key == "" or saved_by_path[source_key] ~= nil then
                parse_error = true
            else
                saved_by_path[source_key] = saved
            end
        end
    end
    if parse_error then
        index.last_reason = "索引数据源记录无效"
        return false
    end

    local manifest_changed = saved_source_count ~= #state.sources
    local progress_current_source = nil
    local reused_source_count = 0
    index.build_required = {}
    for i = 1, #state.sources do
        local current = state.sources[i]
        local saved = saved_by_path[log_tools.apt_index_source_key(current.path)]
        local reusable = saved ~= nil
            and tonumber(saved.size) == tonumber(current.size)
            and tonumber(saved.signature) == tonumber(current.signature)
        if reusable then
            index.source_entries[i] = saved.entries
            index.source_complete[i] = saved.complete == true
            index.source_resume_offsets[i] = 0
            if saved.complete == true then
                reused_source_count = reused_source_count + 1
            elseif saved.saved_index == saved_source_index then
                index.source_resume_offsets[i] = math.min(
                    math.max(0, tonumber(saved_resume_offset) or 0),
                    tonumber(current.size) or 0
                )
                progress_current_source = i
            else
                -- 只有 PROGRESS 指向的数据源具备可证明安全的续建边界。
                index.source_entries[i] = {}
            end
            if saved.saved_index ~= i or saved.label ~= current.label then manifest_changed = true end
        else
            index.source_entries[i] = {}
            index.source_complete[i] = false
            index.source_resume_offsets[i] = 0
            manifest_changed = true
        end
        index.build_required[i] = index.source_complete[i] ~= true
    end

    index.reused_source_count = reused_source_count
    local pending_source_count = log_tools.refresh_apt_index_pending_count()
    index.complete = pending_source_count == 0
    index.incremental_update = pending_source_count > 0 and reused_source_count > 0
    index.build_source_index = log_tools.find_next_apt_index_source(1)
    if index.build_source_index == progress_current_source then
        index.build_resume_offset = index.source_resume_offsets[index.build_source_index] or 0
    else
        index.build_resume_offset = 0
    end
    index.build_partial_offset = index.build_resume_offset
    log_tools.rebuild_apt_index_entries()
    index.dirty = manifest_changed or pending_source_count > 0 or saved_complete ~= index.complete
    index.last_status = index.complete and "ready" or "building"
    if pending_source_count > 0 then
        index.last_reason = string.format(
            "增量索引：复用 %d 个数据源，仅扫描 %d 个变化数据源",
            reused_source_count,
            pending_source_count
        )
    elseif manifest_changed then
        index.last_reason = "地景优先级已更新，现有数据源索引全部复用"
    else
        index.last_reason = ""
    end
    index.notice_build_required = pending_source_count > 0
    index.notice_completion_pending = false
    index.notice_complete_until = 0
    return true
end

function log_tools.request_apt_index_save(reason)
    local index = log_tools.runway_state.index
    if index.dirty ~= true then return false end
    index.save_requested = true
    if reason ~= nil then index.last_reason = tostring(reason) end
    return true
end

function log_tools.abort_apt_index_save(reason, now)
    local index = log_tools.runway_state.index
    if index.save_file ~= nil then pcall(function() index.save_file:close() end) end
    index.save_file = nil
    index.save_active = false
    ...
    index.save_source_index = 1
    index.save_entry_key = nil
    index.last_checkpoint_at = now or index.last_checkpoint_at
    index.save_retry_after = (now or index.last_checkpoint_at or 0) + 30.0
    index.last_status = "save_error"
    index.last_reason = tostring(reason or "索引缓存写入失败")
end

function log_tools.begin_apt_index_save(now)
    local state = log_tools.runway_state
    local index = state.index
    if index.save_active or index.dirty ~= true then
        index.save_requested = false
        return false
    end
    log_tools.pause_apt_index_build()
    local file, open_error = io.open(index.temp_path, "wb")
    if file == nil then
        log_tools.abort_apt_index_save(open_error or "无法创建索引临时文件", now)
        return false
    end
    index.save_file = file
    local write_ok, write_error = file:write(
        "LMM_APT_INDEX\t", tostring(log_tools.runway_config.index_version), "\n"
    )
    if write_ok == nil then
        log_tools.abort_apt_index_save(write_error, now)
        return false
    end
    for i = 1, #state.sources do
        local source = state.sources[i]
        write_ok, write_error = file:write(string.format(
            "SOURCE\t%d\t%d\t%d\t%d\t%s\t%s\n",
            i,
            math.floor(tonumber(source.size) or 0),
            math.floor(tonumber(source.signature) or 0),
            index.source_complete[i] == true and 1 or 0,
            log_tools.apt_index_hex_encode(source.path),
            log_tools.apt_index_hex_encode(source.label)
        ))
        if write_ok == nil then
            log_tools.abort_apt_index_save(write_error, now)
            return false
    ...
    write_ok, write_error = file:write(string.format(
        "PROGRESS\t%d\t%d\t%d\n",
        index.complete and 1 or 0,
        math.floor(index.build_source_index or 1),
        math.floor(index.build_resume_offset or 0)
    ))
    if write_ok == nil then
        log_tools.abort_apt_index_save(write_error, now)
        return false
    end
    index.save_active = true
    index.save_requested = false
    index.save_phase = "entries"
    index.save_source_index = 1
    index.save_entry_key = nil
    return true
end

function log_tools.finish_apt_index_save(now)
    local index = log_tools.runway_state.index
    local write_ok, write_error = index.save_file:write("END\n")
    if write_ok == nil then
        log_tools.abort_apt_index_save(write_error, now)
        return false
    end
    local close_ok, close_error = index.save_file:close()
    index.save_file = nil
    if close_ok == nil then
        log_tools.abort_apt_index_save(close_error, now)
        return false
    end

    local old_file = io.open(index.path, "rb")
    local had_old_file = old_file ~= nil
    if old_file ~= nil then old_file:close() end
    if had_old_file then
        os.remove(index.backup_path)
        local moved_old = os.rename(index.path, index.backup_path)
        if moved_old == nil then
            log_tools.abort_apt_index_save("无法轮换旧索引缓存", now)
            return false
        end
    end
    local installed, install_error = os.rename(index.temp_path, index.path)
    if installed == nil then
        if had_old_file then os.rename(index.backup_path, index.path) end
        log_tools.abort_apt_index_save(install_error or "无法安装新索引缓存", now)
        return false
    end
    if had_old_file then os.remove(index.backup_path) end
    index.save_active = false
    index.save_phase = "idle"
    index.save_source_index = 1
    index.save_entry_key = nil
    index.dirty = false
    index.last_checkpoint_at = now or 0
    index.last_status = index.complete and "ready" or "building"
    index.last_reason = ""
    index.save_retry_after = 0
    if index.complete and index.notice_completion_pending then
        index.notice_completion_pending = false
        index.notice_complete_until = (now or 0)
            + log_tools.runway_config.index_complete_notice_seconds
    end
    return true
end

function log_tools.process_apt_index_save(now)
    local index = log_tools.runway_state.index
    if not index.save_active or index.save_file == nil then return false end
    local line_limit = math.min(
        log_tools.runway_config.index_save_lines_per_step,
        math.max(1, index.adaptive_chunks or 1)
            * log_tools.runway_config.index_save_lines_per_chunk
    )
    local written = 0
    local output = {}
    local reached_end = false
    while written < line_limit do
        local source_index = tonumber(index.save_source_index) or 1
        if source_index > #log_tools.runway_state.sources then
            reached_end = true
            break
        end
        local source_entries = index.source_entries[source_index] or {}
        local next_key, offset = next(source_entries, index.save_entry_key)
        if next_key == nil then
            index.save_source_index = source_index + 1
            index.save_entry_key = nil
        else
            output[#output + 1] = string.format(
                "SOURCE_AIRPORT\t%d\t%s\t%d\n",
                source_index,
                tostring(next_key),
                math.floor(tonumber(offset) or 0)
            )
            index.save_entry_key = next_key
            written = written + 1
        end
    end
    if #output > 0 then
        local write_ok, write_error = index.save_file:write(table.concat(output))
        if write_ok == nil then
            log_tools.abort_apt_index_save(write_error, now)
            return false
        end
    end
    if reached_end then return log_tools.finish_apt_index_save(now) end
    return false
end

function log_tools.record_apt_index_line(line, byte_offset, source_index)
    if tostring(line or ""):match("^%s*1%s") == nil then return false end
    local fields = log_tools.apt_tokens(line)
    local airport_id = string.upper(trim_text(fields[5] or ""))
    if airport_id == "" then return false end
    local index = log_tools.runway_state.index
    local source_entries = index.source_entries[source_index]
    if source_entries == nil then
        source_entries = {}
        index.source_entries[source_index] = source_entries
    end
    -- 同一个 apt.dat 内保留首次出现；跨数据源优先级在分片完成后统一合并。
    if source_entries[airport_id] == nil then source_entries[airport_id] = byte_offset end
    return true
end

function log_tools.update_apt_index_budget(frame_period_sec)
    local index = log_tools.runway_state.index
    local period = tonumber(frame_period_sec) or 0
    local raw_fps = period > 0.001 and period < 1.0 and (1.0 / period) or 0
    if raw_fps <= 0 then raw_fps = index.fps_ema > 0 and index.fps_ema or 30 end
    raw_fps = math.max(5, math.min(240, raw_fps))
    index.raw_fps = raw_fps

    if index.fps_ema <= 0 then
        -- 初次进入时最多从 30 FPS 档起步，防止单个异常高帧率样本立即拉满扫描预算。
        index.fps_ema = math.min(raw_fps, 30)
    else
        -- 掉帧时快速降档，帧率恢复时缓慢升档，避免扫描负载在阈值附近来回跳变。
        local alpha = raw_fps < index.fps_ema and 0.25 or 0.06
        index.fps_ema = index.fps_ema + (raw_fps - index.fps_ema) * alpha
    end

    local fps = index.fps_ema
    if fps < 20 then
        index.adaptive_tier = "PROTECT"
        index.adaptive_chunks = 1
        index.adaptive_interval = 0.40
    elseif fps < 30 then
        index.adaptive_tier = "LOW"
        index.adaptive_chunks = 1
        index.adaptive_interval = 0.15
    elseif fps < 45 then
        index.adaptive_tier = "NORMAL"
        index.adaptive_chunks = 2
        index.adaptive_interval = 0.10
    elseif fps < 60 then
        index.adaptive_tier = "HIGH"
        index.adaptive_chunks = 3
        index.adaptive_interval = 0.075
    else
        index.adaptive_tier = "MAX_SAFE"
        index.adaptive_chunks = 4
        index.adaptive_interval = 0.05
    end
    index.adaptive_chunks = math.min(
        index.adaptive_chunks,
        log_tools.runway_config.index_max_chunks_per_step
    )
    index.adaptive_interval = math.max(
        index.adaptive_interval,
        log_tools.runway_config.index_min_step_interval_seconds
    )
    return index.adaptive_chunks, index.adaptive_interval, index.fps_ema
end

function log_tools.mark_apt_index_complete()
    local state = log_tools.runway_state
    local index = state.index
    if log_tools.refresh_apt_index_pending_count() > 0 then
        index.complete = false
        index.build_source_index = log_tools.find_next_apt_index_source(1)
        index.build_resume_offset = index.source_resume_offsets[index.build_source_index] or 0
        index.build_partial_offset = index.build_resume_offset
    ...
    local should_notify = index.notice_build_required == true
    index.complete = true
    index.build_source_index = #state.sources + 1
    index.build_resume_offset = 0
    index.build_partial = ""
    index.build_partial_offset = 0
    index.dirty = true
    index.last_status = "ready"
    index.notice_build_required = false
    if should_notify then
        -- 内存索引此时已经可用；缓存继续在后台原子落盘，不再延长“初始化中”提示。
        index.notice_completion_pending = false
        index.notice_complete_until = (index.last_update_now or 0)
            + log_tools.runway_config.index_complete_notice_seconds
    end
    index.debug_activity_until = math.max(
        index.debug_activity_until or 0,
        (index.last_update_now or 0) + log_tools.runway_config.index_debug_hold_seconds
    )
    log_tools.request_apt_index_save("索引构建完成")
    return true
end

function log_tools.invalidate_apt_index_source(source_index, reason)
    local state = log_tools.runway_state
    local index = state.index
    local numeric_source_index = tonumber(source_index) or 0
    if state.sources[numeric_source_index] == nil then return false end
    local was_complete = index.complete == true
    log_tools.pause_apt_index_build()
    if was_complete then index.build_required = {} end
    index.build_required[numeric_source_index] = true
    index.source_entries[numeric_source_index] = {}
    index.source_complete[numeric_source_index] = false
    index.source_resume_offsets[numeric_source_index] = 0
    log_tools.rebuild_apt_index_entries()
    index.complete = false
    index.dirty = true
    index.notice_build_required = true
    index.notice_completion_pending = false
    index.build_source_index = log_tools.find_next_apt_index_source(1)
    index.build_resume_offset = index.source_resume_offsets[index.build_source_index] or 0
    index.build_partial = ""
    index.build_partial_offset = index.build_resume_offset
    index.last_step_at = -1000
    index.last_status = "building"
    index.last_reason = tostring(reason or "单个 apt.dat 索引需要重建")
    log_tools.refresh_apt_index_pending_count()
    local reusable_sources = 0
    for i = 1, #state.sources do
        if index.source_complete[i] == true then reusable_sources = reusable_sources + 1 end
    end
    index.reused_source_count = reusable_sources
    index.incremental_update = reusable_sources > 0
    return true
end

function log_tools.open_apt_index_build_source()
    local state = log_tools.runway_state
    local index = state.index
    if index.complete then return false end
    index.build_source_index = log_tools.find_next_apt_index_source(index.build_source_index)
    if index.build_source_index > #state.sources then
        log_tools.mark_apt_index_complete()
        return false
    end
    local source = state.sources[index.build_source_index]
    index.build_resume_offset = index.source_resume_offsets[index.build_source_index] or 0
    local file = io.open(source.path, "rb")
    if file == nil then
        index.last_status = "build_error"
        index.last_reason = "无法读取 apt.dat: " .. tostring(source.path)
        return false
    end
    local positioned = file:seek("set", index.build_resume_offset or 0)
    if positioned == nil then
        -- 只丢弃当前损坏分片；其他已验证 apt.dat 继续复用。
        index.source_entries[index.build_source_index] = {}
        index.source_resume_offsets[index.build_source_index] = 0
        positioned = file:seek("set", 0)
        index.last_reason = "当前数据源续建位置失效，仅从该 apt.dat 文件头重建"
        if positioned == nil then
            file:close()
            index.last_status = "build_error"
            return false
    ...
    index.build_file = file
    index.build_partial = ""
    index.build_partial_offset = positioned
    index.build_resume_offset = positioned
    index.source_resume_offsets[index.build_source_index] = positioned
    return true
end

function log_tools.process_apt_index_chunk()
    local state = log_tools.runway_state
    local index = state.index
    if index.complete then return true end
    if index.build_file == nil and not log_tools.open_apt_index_build_source() then
        return index.complete
    end
    if index.build_file == nil then return false end

    local chunk = index.build_file:read(log_tools.runway_config.index_chunk_bytes)
    if chunk == nil or chunk == "" then
        if index.build_partial ~= "" then
            log_tools.record_apt_index_line(
                index.build_partial,
                index.build_partial_offset,
                index.build_source_index
            )
        end
        index.build_file:close()
        index.build_file = nil
        local completed_source_index = index.build_source_index
        index.source_complete[completed_source_index] = true
        index.source_resume_offsets[completed_source_index] = 0
        log_tools.merge_apt_index_source(completed_source_index)
        index.build_source_index = log_tools.find_next_apt_index_source(completed_source_index + 1)
        index.build_resume_offset = 0
        index.build_partial = ""
        index.build_partial_offset = 0
        index.dirty = true
    ...
    local data = index.build_partial .. chunk
    local buffer_offset = index.build_partial_offset
    local line_start = 1
    while true do
        local newline_at = data:find("\n", line_start, true)
        if newline_at == nil then break end
        local line = data:sub(line_start, newline_at - 1):gsub("\r$", "")
        log_tools.record_apt_index_line(
            line,
            buffer_offset + line_start - 1,
            index.build_source_index
        )
        line_start = newline_at + 1
    end
    index.build_partial = data:sub(line_start)
    index.build_partial_offset = buffer_offset + line_start - 1
    index.build_resume_offset = index.build_partial_offset
    index.source_resume_offsets[index.build_source_index] = index.build_resume_offset
    index.dirty = true
    index.debug_last_bytes = #chunk
    index.last_status = "building"
    return false
end

function log_tools.update_apt_index(now, radio_alt_ft, on_ground, local_vy_mps, gs_kt, frame_period_sec)
    local state = log_tools.runway_state
    local index = state.index
    local config = log_tools.runway_config
    index.last_update_now = now or 0
    log_tools.update_apt_index_budget(frame_period_sec)
    if runtime_state.runway_detection_enabled ~= true or #state.sources == 0 then return false end
    if state.active
        or (landing_analysis.phase ~= "idle" and landing_analysis.phase ~= "complete")
        or bounce_state.monitoring == true
        or landing_jobs.context_pending == true
        or landing_jobs.log_pending == true
        or landing_jobs.log_write ~= nil then
        return false
    end

    local safe_on_ground = on_ground ~= 0 and gs_kt <= config.index_ground_max_speed_kt
    local safe_in_cruise = on_ground == 0
        and radio_alt_ft >= config.index_cruise_min_agl_ft
        and math.abs(local_vy_mps) <= config.index_cruise_max_vertical_speed_mps
    if not safe_on_ground and not safe_in_cruise then return false end
    if now - index.last_step_at < index.adaptive_interval then return false end
    index.last_step_at = now

    if index.save_active then
        return log_tools.process_apt_index_save(now)
    end
    if index.save_requested then
        if now < (index.save_retry_after or 0) then return false end
        if log_tools.begin_apt_index_save(now) then
            return log_tools.process_apt_index_save(now)
        end
        return false
    end
    if index.complete then
        if index.dirty then log_tools.request_apt_index_save("保存完整索引") end
        return true
    end

    for i = 1, index.adaptive_chunks do
        if log_tools.process_apt_index_chunk() then break end
    end
    index.debug_activity_until = now + config.index_debug_hold_seconds
    if index.dirty
        and not index.complete
        and now - index.last_checkpoint_at >= config.index_checkpoint_seconds then
        log_tools.request_apt_index_save("保存渐进建库进度")
    end
    return index.complete
end

function log_tools.open_indexed_apt_source(entry)
    local state = log_tools.runway_state
    local source = state.sources[entry and entry.source_index or 0]
    if source == nil then
        log_tools.rebuild_apt_index_entries()
        state.index.last_reason = "索引条目引用了无效数据源，已忽略该条目"
        return false
    end
    local file = io.open(source.path, "rb")
    if file == nil then
        log_tools.invalidate_apt_index_source(entry.source_index, "索引数据源暂时不可读，仅安排重建该分片")
        return false
    end
    local positioned = file:seek("set", entry.offset or -1)
    local header_line = positioned ~= nil and file:read("*l") or nil
    local fields = header_line ~= nil and log_tools.apt_tokens(header_line) or {}
    local indexed_airport = string.upper(trim_text(fields[5] or ""))
    local valid_header = header_line ~= nil
        and header_line:match("^%s*1%s") ~= nil
        and indexed_airport == state.target_airport
    if not valid_header or file:seek("set", entry.offset or -1) == nil then
        file:close()
        log_tools.invalidate_apt_index_source(entry.source_index, "索引字节位置校验失败，仅安排重建该分片")
        return false
    end
    state.source_index = entry.source_index
    state.source = source
    state.file = file
    state.partial = ""
    ...
    state.runways = {}
    state.metadata = {}
    state.indexed_lookup = true
    state.scan_route = "INDEX_SEEK"
    log_tools.update_runway_scan_debug("indexed_seek", nil, "")
    return true
end

function log_tools.update_runway_scan_debug(status, now, reason)
    if DEBUG_MODE ~= true then return end
    local state = log_tools.runway_state
    local debug_state = state.scan_debug
    local display_now = tonumber(now)
    if display_now == nil then display_now = lmm_get_float("running_time_sec") end
    local byte_offset = 0
    if state.file ~= nil then
        local seek_ok, position = pcall(function() return state.file:seek() end)
        if seek_ok and tonumber(position) ~= nil then byte_offset = tonumber(position) end
    end
    debug_state.visible_until = display_now + log_tools.runway_config.runway_debug_hold_seconds
    debug_state.status = tostring(status or "reading")
    debug_state.target_airport = tostring(state.target_airport or "")
    debug_state.mode = tostring(state.mode or "idle")
    debug_state.source_index = tonumber(state.source_index) or 0
    debug_state.source_count = #state.sources
    debug_state.source_label = state.source and tostring(state.source.label or "") or ""
    debug_state.byte_offset = byte_offset
    debug_state.indexed_lookup = state.indexed_lookup == true
    debug_state.route = tostring(state.scan_route or "NONE")
    debug_state.reason = tostring(reason or "")
end

function log_tools.cancel_runway_resolver(reason)
    local state = log_tools.runway_state
    if state.active then log_tools.update_runway_scan_debug("cancelled", nil, reason) end
    ...
    state.metadata = {}
    state.mode = "idle"
    state.indexed_lookup = false
    state.scan_route = "NONE"
...
function log_tools.finish_runway_resolver(status, reason)
    ...
    log_tools.update_runway_scan_debug("finished:" .. tostring(status or "unavailable"), nil, reason)
    if state.file ~= nil then pcall(function() state.file:close() end) end
    state.file = nil
    state.active = false
    state.source = nil
...
function log_tools.finish_airport_block()
    ...
    if best.geometry and not log_tools.dev.route then
        log_tools.dev.route=log_tools.rollout_module.new(best.geometry,nil,best.runway_id,state.source and state.source.path)
        if log_tools.dev.route then log_tools.dev.route.session_id=log_tools.recording.id end
    end
...
function log_tools.open_next_apt_source()
    ...
    while state.source_index < #state.sources do
        ...
        state.metadata = {}
        state.indexed_lookup = false
        state.scan_route = "SEQUENTIAL"
        if state.file ~= nil then
            log_tools.update_runway_scan_debug("sequential_read", nil, "")
            return true
        end
    end
...
function log_tools.start_runway_resolver(airport_id, now, mode)
    ...
    local indexed_entry = state.index.entries[target_airport]
    if indexed_entry ~= nil
        and log_tools.apt_index_entry_is_trusted(indexed_entry)
        and log_tools.open_indexed_apt_source(indexed_entry) then
        return true
    end
    -- 完整且通过源文件校验的索引中没有该机场时，无需再从数百 MB 文件头扫描。
    if state.index.complete then
        log_tools.finish_runway_resolver("unavailable", "完整 apt.dat 索引中未找到该机场")
        return false
    end
    -- 首次建库尚未覆盖该机场时保留旧顺序扫描，保证首次启用仍有可用回退。
...
function log_tools.process_runway_resolver(now)
    ...
    log_tools.update_runway_scan_debug("reading", now, "")
    if chunk == nil then
...
local function landing_context_short_text()
    ...
    if airport == "识别中" then
        return string.format(log_tools.popup_text("%s | 机场信息计算中", "%s | Resolving airport data"), aircraft)
    end
    if airport == "UNKNOWN" then
        return string.format(log_tools.popup_text("%s | 机场未识别", "%s | Airport not identified"), aircraft)
    end
    if landing_context.runway_detected then
    ...
    if landing_context.runway_status == "pending" or landing_context.runway_status == "scanning" then
        return string.format(log_tools.popup_text("%s | %s | 跑道识别中", "%s | %s | Resolving runway"), aircraft, airport)
    end
...
local function format_roll_text(roll_deg)
    ...
    if abs_roll < 0.05 then
        return log_tools.popup_text("横滚 LEVEL 0.0°", "Roll LEVEL 0.0 deg")
    elseif roll_deg < 0 then
        return string.format(log_tools.popup_text("横滚 L %.1f°", "Roll L %.1f deg"), abs_roll)
    end
    return string.format(log_tools.popup_text("横滚 R %.1f°", "Roll R %.1f deg"), abs_roll)
end
...
local function status_color(status)
    local color = log_tools.popup_style.colors[status]
        or log_tools.popup_style.colors.NICE
...
function log_tools.popup_status_text(status)
    if status == "UNSTABLE" then return log_tools.popup_text("UNSTABLE 不良落地", "UNSTABLE Adverse landing") end
    if status == "ATTENTION" then return log_tools.popup_text("Attention 需注意", "Attention Review advised") end
    if status == "STABLE" then return log_tools.popup_text("Stable 稳定扎实落地", "Stable Solid landing") end
    return log_tools.popup_text("Nice 轻柔接地", "Nice Soft touchdown")
end

function log_tools.popup_flare_trend_text(value)
    if log_tools.popup_language() ~= "en" then return value end
    local english = {
...
local function refresh_popup_cache()
    ...
    popup_cache.lines[4] = string.format(log_tools.popup_text("迎角 %.1f° | %s", "AoA %.1f deg | %s"), landing_aoa_deg, format_roll_text(landing_roll_deg))
    local flare_trend = log_tools.popup_flare_trend_text(flare_analysis.trend_text)
    if flare_analysis.valid then
        popup_cache.lines[5] = string.format(log_tools.popup_text("拉平曲率 %.1f | %s", "Flare curvature %.1f | %s"), flare_analysis.metric, flare_trend)
    else
        popup_cache.lines[5] = log_tools.popup_text("拉平曲率 -- | ", "Flare curvature -- | ") .. flare_trend
    end
    popup_cache.lines[6] = string.format(
        log_tools.popup_text("风 %03d/%dkt %s", "Wind %03d/%dkt %s"),
        round_num(normalize_deg(landing_wind_heading_deg)),
        round_num(landing_wind_speed_kts),
        log_tools.relative_wind_side(landing_wind_heading_deg, landing_heading_deg, log_tools.popup_language())
    )
    ...
    popup_cache.centerline_text = log_tools.popup_centerline_text()
    if bounce_state.detected then
        if bounce_state.second_g_ready then
            popup_cache.bounce_text = string.format(log_tools.popup_text("发生弹跳 | 二次 %+d fpm / +%.2fG", "Bounce detected | 2nd %+d fpm / +%.2fG"), bounce_state.second_fpm, bounce_state.second_curve_g)
        else
            popup_cache.bounce_text = log_tools.popup_text("发生弹跳 | 正在分析第二次触地", "Bounce detected | Analysing second touchdown")
        end
    else
        popup_cache.bounce_text = ""
    end
    if landing_report_notice.ready then
        landing_report_notice.text = log_tools.popup_text(
            "落地报告已生成",
            "Landing report generated"
        )
    end
end
...
local function reset_flare_trace()
    ...
    flare_trace.end_time = 0
    flare_trace.touch_reference_ft = 0
    flare_trace.post_touch_active = false
    flare_trace.post_touch_deadline = 0
    flare_trace.ground_stable_since = 0
    flare_trace.last_on_ground = 0
    flare_trace.airborne_episode_count = 0
    flare_trace.touch_count = 0
    flare_trace.end_reason = "等待接地"
    flare_trace.touch_sample_index = 0
    flare_trace.touch_bucket_index = 0
    ...
    flare_trace.control_sample_count = 0
    ...
    flare_analysis.trace_duration_seconds = 0
    flare_analysis.post_touch_duration_seconds = 0
...
local function reset_bounce_state()
    ...
    bounce_state.recording_id = nil
    bounce_state.last_time = nil
    bounce_state.low_speed_since = nil
    bounce_state.climb_since = nil
    bounce_state.first_touch_height = 0
...
local function start_flare_trace(now)
    -- 飞机可能在同一 X-Plane 会话内切换；每次进入 100 ft 采样重新绑定机型专用接口。
    LMM_CONTROL_REFS.refresh_engine_context()
    for _, spec in pairs(LMM_CONTROL_REFS.inputs) do
        spec.selected_candidate = nil
    end
...
local function add_flare_trace_sample(
    ...
    slot.roll_deg = roll_deg
    slot.g_normal = tonumber(current_g) or 1
    slot.on_ground = on_ground == 1 and 1 or 0
    LMM_CONTROL_REFS.capture(slot)
    flare_trace.next_sample_time = now + FLARE_CONFIG.sample_interval_seconds
end

local function finish_flare_trace(now, reason)
    flare_trace.active = false
    flare_trace.post_touch_active = false
    flare_trace.complete = true
    flare_trace.end_time = now
    flare_trace.end_reason = reason or "接地过程记录完成"

    -- The short 100 ft excerpt may finish; bounce monitoring belongs to the
    -- entire landing/rollout session and must not end with this display buffer.
end

local function begin_flare_post_touch(
    now,
    touchdown_height_ft,
    local_vy_mps,
    vvi_fpm,
    ...
    flare_trace.active = true
    flare_trace.complete = false
    flare_trace.post_touch_active = true
    flare_trace.touch_time = now
    flare_trace.end_time = 0
    flare_trace.post_touch_deadline = now + FLARE_CONFIG.post_touch_max_seconds
    flare_trace.ground_stable_since = now
    flare_trace.last_on_ground = 1
    flare_trace.airborne_episode_count = 0
    flare_trace.touch_count = 1
    flare_trace.end_reason = "正在记录接地过程"
    ...
    -- 接地帧单独写入并冻结索引，避免 10 Hz 节拍恰好跨过第一次接触。
    ...
    flare_trace.touch_sample_index = flare_trace.count
end
...
local function update_flare_trace(
    ...
    if flare_trace.active == true and flare_trace.post_touch_active == false then
    ...
    if flare_trace.active == true
        and (on_ground == 0 or flare_trace.post_touch_active == true)
        and now >= flare_trace.next_sample_time then
        add_flare_trace_sample(
            now,
            radio_alt_ft,
            local_vy_mps,
    ...
    if flare_trace.post_touch_active == true then
        if on_ground ~= flare_trace.last_on_ground then
            if flare_trace.last_on_ground == 1 and on_ground == 0 then
                flare_trace.airborne_episode_count = flare_trace.airborne_episode_count + 1
                flare_trace.ground_stable_since = -1
            elseif flare_trace.last_on_ground == 0 and on_ground == 1 then
                flare_trace.touch_count = flare_trace.touch_count + 1
                flare_trace.ground_stable_since = now
            end
            flare_trace.last_on_ground = on_ground
        elseif on_ground == 1 and flare_trace.ground_stable_since < 0 then
            flare_trace.ground_stable_since = now
        end

        if on_ground == 1
            and flare_trace.ground_stable_since >= 0
            and now - flare_trace.ground_stable_since
                >= FLARE_CONFIG.post_touch_ground_seconds then
            finish_flare_trace(now, flare_trace.airborne_episode_count > 0
                and "最后一次接地后连续记录满2秒"
                or "首次接地后连续记录满2秒")
        elseif now >= flare_trace.post_touch_deadline then
            finish_flare_trace(now, "达到接地过程8秒安全上限")
        end
    end
end
...
local function analyze_flare_curve()
    ...
    flare_analysis.trace_duration_seconds = math.max(
        0,
        (flare_trace.end_time > 0 and flare_trace.end_time or flare_trace.touch_time)
            - flare_trace.start_time
    )
    flare_analysis.post_touch_duration_seconds = math.max(
        0,
        (flare_trace.end_time > 0 and flare_trace.end_time or flare_trace.touch_time)
            - flare_trace.touch_time
    )
    ...
    for i = 1, flare_trace.count do
        ...
        if sample.t >= flare_trace.start_time then
            ...
            flare_trace.control_sample_count = flare_trace.control_sample_count + 1
            ...
            bucket.g_normal = bucket.g_normal + sample.g_normal
            bucket.ground_ratio = bucket.ground_ratio + sample.on_ground
        end
    end
    ...
    for i = 1, FLARE_CONFIG.max_buckets do
        ...
        if source.count > 0 then
            ...
            if target ~= source then
                ...
                target.g_normal = source.g_normal
                target.ground_ratio = source.ground_ratio
            end
            ...
            target.g_normal = target.g_normal / count
            target.ground_ratio = target.ground_ratio / count
        end
    end
    ...
    -- v1.1.5 起聚合轨迹在首次接地后继续延伸；因此触地桶不再等同于末桶。
    -- 选择首个不早于首次接地的桶作为接地锚点，并只对该桶冻结评分使用的 FPM/G。
    local touch_bucket_index = compact_count
    for i = 1, compact_count do
        if flare_trace.buckets[i].t >= flare_trace.touch_time then
            touch_bucket_index = i
            break
        end
    end
    local touch_bucket = flare_trace.buckets[touch_bucket_index]
    ...
    touch_bucket.g_normal = landing_touch_g
    touch_bucket.ground_ratio = 1
    flare_trace.touch_bucket_index = touch_bucket_index
    flare_trace.bucket_count = compact_count

    if touch_bucket_index < FLARE_CONFIG.min_buckets then
        flare_analysis.trend_text = "首次接地前拉平聚合点不足"
        flare_analysis.calculation_ms = (os.clock() - started) * 1000
        return
    end

    local entry_fpm = flare_trace.buckets[1].selected_fpm
    local touch_fpm = flare_trace.buckets[touch_bucket_index].selected_fpm
...
local function analyze_landing_impulse()
    ...
    -- FPM/G 主链已经完成，但轨迹需继续等待接地过程记录结束。
    landing_analysis.phase = "wait_flare_trace"
    landing_analysis.analysis_ms = landing_analysis.analysis_ms + (os.clock() - started) * 1000
end
...
local function position_label(position_id)
    local names = {top_left="左上",top_center="上方居中",top_right="右上",
        middle_left="左侧居中",center="正中",middle_right="右侧居中",
        bottom_left="左下",bottom_center="下方居中",bottom_right="右下"}
    for i = 1, #POSITION_OPTIONS do
        if POSITION_OPTIONS[i].id == position_id then return log_tools.settings_text(names[position_id], POSITION_OPTIONS[i].label) end
    end
    return "Middle left"
end
...
local function calculate_popup_position(screen_w, screen_h, panel_w, panel_h)
    ...
    local cfg=log_tools.dev.config
    if cfg.popup_free_x>=0 and cfg.popup_free_y>=0 then
        x=cfg.popup_free_x*math.max(0,screen_w-panel_w)
        y=cfg.popup_free_y*math.max(0,screen_h-panel_h)
    end
    -- 九宫格负责默认锚点；用户微调只作为独立像素偏移叠加，不改变锚点本身。
    x = x + (runtime_state.popup_offset_x or 0)
    y = y + (runtime_state.popup_offset_y or 0)
...
function log_tools.native_shutdown()
    log_tools.dev:destroy()
    if log_tools.native_ui.instance then
        log_tools.native_ui.instance:destroy()
        log_tools.native_ui.instance = nil
    end
end

function log_tools.native_initialize()
    local ui = log_tools.native_ui
    if ui.attempted then return end
    ui.attempted = true
    if ui.force_compatibility then
        ui.error = "Legacy UI compatibility edition (no SDK 440 font calls)"
        return
    end
    local ok, result = pcall(function()
        assert(type(do_on_exit) == "function", "FlyWithLua cleanup callback unavailable")
        local directory = join_path(LMM_BASE_DIRECTORY, "LMM_UI_119") .. PATH_SEPARATOR
        local loader, err = loadfile(directory .. "native_popup.lua")
        assert(loader, err)
        local module = loader(directory)
        return module.new({system = SYSTEM, root = SYSTEM_DIRECTORY or "",
            assets = directory})
    end)
    if ok then ui.instance = result end
    ui.error = ok and (result.error or "") or tostring(result)
    refresh_popup_cache()
    if logMsg then
...
function log_tools.native_popup(visible, now)
    local ui = log_tools.native_ui
    if not ui.instance then return false end
    if not ui.instance.ready then
        ui.error = ui.instance.error or "Native draw callback failed"
        log_tools.native_shutdown()
        refresh_popup_cache()
        if logMsg then logMsg("[StarLux LMM] Native popup stopped; legacy fallback: " .. ui.error) end
        return false
    end
    if log_tools.popup_style.renderer == "legacy" then ui.instance:hide(); return false end
    local ok, err = pcall(function()
        if not visible then ui.instance:hide(); return end
        if landing_report_notice.ready and landing_report_notice.flash_started_at <= 0 then
            log_tools.start_report_flash(now)
            show_until = math.max(show_until, landing_report_notice.flash_until_time)
        end
        local lines = {}
        for i = 1, 7 do lines[#lines + 1] = popup_cache.lines[i] end
        if popup_cache.centerline_text ~= "" then lines[#lines + 1] = popup_cache.centerline_text end
        if bounce_state.detected then lines[#lines + 1] = popup_cache.bounce_text end
        if landing_surface.wet_warning and landing_status == "NICE" then
            lines[#lines + 1] = popup_cache.warning_text
        end
        local flashing, lit = log_tools.report_flash_state(now)
        local hidden_line
        if flashing then
            lines[#lines + 1] = landing_report_notice.text
            if not lit then hidden_line = #lines end
        end
        if DEBUG_MODE then
            lines[#lines + 1] = string.format("DBG FPM source:%s", landing_analysis.flare_fpm_source)
            lines[#lines + 1] = string.format("DBG G pk:%.2f curve:%.2f eq:%.2f", debug_data.peak_g, debug_data.robust_g, debug_data.expected_max_g)
            lines[#lines + 1] = string.format("DBG err:%.0f%% %s %.2fms", debug_data.consistency_error * 100, debug_data.confidence, debug_data.analysis_ms)
        end
        local r, g, b = status_color(landing_status)
        local tr, tg, tb = log_tools.popup_text_color(r, g, b)
        ui.instance:present({lines = lines, hidden_line = hidden_line,
            size = log_tools.popup_style.font_px, weight = log_tools.popup_style.font_weight,
            vertical = POPUP_LAYOUT == "vertical", color = {r, g, b},
            text_color = {tr, tg, tb}, alpha = panel_alpha(), position = calculate_popup_position,
            on_resize=function(size,finished)
                log_tools.popup_style.font_px=size
                if finished then log_tools.dev.pending="save" end
            end,
            on_move=function(x,y,finished)
                log_tools.dev.config.popup_free_x=x; log_tools.dev.config.popup_free_y=y
                runtime_state.popup_offset_x=0; runtime_state.popup_offset_y=0
                if finished then log_tools.dev.pending="save" end
            end})
    end)
    if not ok then
        ui.instance.ready, ui.instance.error = false, tostring(err)
        return log_tools.native_popup(false, now)
    end
    return true
end

function log_tools.open_native_dialog(id, builder, title, width, height)
    log_tools.native_initialize()
    local host = log_tools.native_ui.instance
    if not host or not host.ready then return false end
    local ok, err = pcall(host.open_dialog, host, id, builder, title, width, height)
    if not ok then
        log_tools.native_ui.dialog_error = tostring(err)
        if logMsg then logMsg("[StarLux LMM] Native dialog: " .. tostring(err)) end
    end
    return ok
end

function log_tools.update_native_dialogs()
    local host = log_tools.native_ui.instance
    if host and host.ready then
        local ok, err = pcall(host.update_dialogs, host)
        if not ok then
            for _, dialog in pairs(host.dialogs or {}) do dialog:destroy() end
            host.dialogs = {}
            if logMsg then logMsg("[StarLux LMM] Native dialog update: " .. tostring(err)) end
        end
    end
end
...
local function save_settings()
    ...
    file:write("# document_language: global zh/en; all plugin UI, TXT and viewer launch language\n")
    file:write("# popup_color_*: red,green,blue values from 0 to 255\n")
    ...
    file:write("popup_offset_x=" .. tostring(runtime_state.popup_offset_x or 0) .. "\n")
    file:write("popup_offset_y=" .. tostring(runtime_state.popup_offset_y or 0) .. "\n")
    file:write("popup_layout=" .. POPUP_LAYOUT .. "\n")
    file:write("panel_opacity=" .. tostring(PANEL_OPACITY_LEVEL) .. "\n")
    file:write("popup_color_nice=" .. log_tools.rgb_setting_text(log_tools.popup_style.colors.NICE) .. "\n")
    file:write("popup_color_stable=" .. log_tools.rgb_setting_text(log_tools.popup_style.colors.STABLE) .. "\n")
    file:write("popup_color_attention=" .. log_tools.rgb_setting_text(log_tools.popup_style.colors.ATTENTION) .. "\n")
    file:write("popup_color_unstable=" .. log_tools.rgb_setting_text(log_tools.popup_style.colors.UNSTABLE) .. "\n")
    file:write("popup_text_color=" .. log_tools.popup_style.text_color_mode .. "\n")
    file:write("popup_font_size=" .. log_tools.popup_style.font_size .. "\n")
    file:write("popup_font_px=" .. tostring(log_tools.popup_style.font_px) .. "\n")
    file:write("popup_renderer=" .. log_tools.popup_style.renderer .. "\n")
    file:write("popup_font_weight=" .. log_tools.popup_style.font_weight .. "\n")
    ...
    log_tools.dev:write_settings(file)
    file:close()
    settings_save_ok = true
    return true
end

function log_tools.set_global_language(language)
    if language ~= "zh" and language ~= "en" then return end
    runtime_state.document_language = language
    refresh_popup_cache()
    ...
    local host = log_tools.native_ui.instance
    if host and host.ready then
        for id, dialog in pairs(host.dialogs or {}) do
            host.api.XPLMSetWindowTitle(dialog.window, id == "settings"
                and log_tools.settings_text("StarLux LMM - 设置", "StarLux LMM - Settings")
                or log_tools.settings_text("StarLux LMM - 落地记录", "StarLux LMM - Landing records"))
        end
    end
    if settings_window then float_wnd_set_title(settings_window, log_tools.settings_text("StarLux LMM - 设置", "StarLux LMM - Settings")) end
    if log_manager_window then float_wnd_set_title(log_manager_window, log_tools.settings_text("StarLux LMM - 落地记录", "StarLux LMM - Landing records")) end
    save_settings()
end
...
local function load_settings()
    ...
    local popup_color_key_count = 0
    local popup_text_color_key_found = false
    local popup_font_size_key_found = false
    local popup_font_weight_key_found = false
    local popup_offset_key_count = 0
    for line in file:lines() do
        local key, value = line:match("^%s*([%w_]+)%s*=%s*(.-)%s*$")
        log_tools.dev:read_setting(key, value)
        ...
        elseif key == "document_language" and (value == "zh" or value == "en") then
            ...
            log_tools.global_language_loaded = true
        elseif key == "interface_language" and not log_tools.global_language_loaded and (value == "zh" or value == "en") then
        ...
        elseif key == "popup_offset_x" then
            local offset = tonumber(value)
            if offset ~= nil then
                runtime_state.popup_offset_x = math.max(-1000, math.min(1000, round_num(offset)))
                popup_offset_key_count = popup_offset_key_count + 1
            end
        elseif key == "popup_offset_y" then
            local offset = tonumber(value)
            if offset ~= nil then
                runtime_state.popup_offset_y = math.max(-1000, math.min(1000, round_num(offset)))
                popup_offset_key_count = popup_offset_key_count + 1
            end
        ...
        elseif key == "popup_color_nice" then
            if log_tools.parse_rgb_setting(value, log_tools.popup_style.colors.NICE) then
                popup_color_key_count = popup_color_key_count + 1
            end
        elseif key == "popup_color_stable" then
            if log_tools.parse_rgb_setting(value, log_tools.popup_style.colors.STABLE) then
                popup_color_key_count = popup_color_key_count + 1
            end
        elseif key == "popup_color_attention" then
            if log_tools.parse_rgb_setting(value, log_tools.popup_style.colors.ATTENTION) then
                popup_color_key_count = popup_color_key_count + 1
            end
        elseif key == "popup_color_unstable" then
            if log_tools.parse_rgb_setting(value, log_tools.popup_style.colors.UNSTABLE) then
                popup_color_key_count = popup_color_key_count + 1
            end
        elseif key == "popup_text_color" then
            if value == "contrast" or value == "white" or value == "black" then
                log_tools.popup_style.text_color_mode = value
                popup_text_color_key_found = true
            end
        elseif key == "popup_font_size" then
            if value == "small" or value == "normal" or value == "large" then
                log_tools.popup_style.font_size = value
                popup_font_size_key_found = true
            end
        elseif key == "popup_font_px" then
            local px = tonumber(value)
            if px and px == px and px >= 10 and px <= 32 then
                log_tools.popup_style.font_px = math.floor(px + 0.5)
                log_tools.popup_style.font_px_loaded = true
            end
        elseif key == "popup_renderer" and (value == "auto" or value == "legacy") then
            log_tools.popup_style.renderer = value
        elseif key == "popup_font_weight" then
            if value == "normal" or value == "medium" or value == "bold" then
                log_tools.popup_style.font_weight = value
                popup_font_weight_key_found = true
            end
    ...
    file:close()
    if not log_tools.popup_style.font_px_loaded then
        log_tools.popup_style.font_px = ({small = 10, normal = 14, large = 18})[log_tools.popup_style.font_size] or 14
        migrated = true
    end
    if not detailed_math_key_found then migrated = true end
    if not runway_detection_key_found then migrated = true end
    if popup_color_key_count < 4 then migrated = true end
    if not popup_text_color_key_found then migrated = true end
    if not popup_font_size_key_found then migrated = true end
    if not popup_font_weight_key_found then migrated = true end
    if popup_offset_key_count < 2 then migrated = true end
        ...
        elseif in_table then
            ...
            if #values >= 3 then
                points[#points + 1] = {
                    ...
                    aoa = values[8],
                    roll = values[9],
                    physical_fpm = values[10],
                    g_normal = values[11],
                    ground_ratio = values[12]
                }
                data_started = true
...
function log_tools.write_viewer_html(report)
    ...
    data_file:write("window.LMM_LANGUAGE_LOCKED = true;\n")
...
log_tools.report_translations = {
    { "等待终端下降率测量", "Awaiting terminal vertical-speed measurement" },
    { "等待接地", "Awaiting touchdown" },
    { "等待计算", "Awaiting calculation" },
    { "等待分析", "Awaiting analysis" },
    { "未检测到持续降雨或湿滑道面", "No persistent rain or wet/slippery surface detected" },
    { "未检测到近期降水或湿滑摩擦", "No recent precipitation or wet/slippery friction detected" },
    { "识别中", "Resolving" },
    { "窗口标记: F5=5ft以下250ms物理备用/VVI参考，F100=接地前100ms物理候选窗，V850=850ms最差VVI诊断，AGL=5ft以下几何锚点，BASE=接地前G参考，IMPACT=第一次压缩，POST=压缩末端速度。", "Window flags: F5=250 ms physical fallback/VVI reference below 5 ft; F100=100 ms pre-touchdown physical candidate; V850=worst VVI over 850 ms; AGL=geometric anchor below 5 ft; BASE=pre-touchdown G baseline; IMPACT=first compression; POST=end-of-compression velocity." },
    { "接地前100ms物理垂直速度(m/s)", "Physical vertical speed in 100 ms before touchdown (m/s)" },
    { "接地 FPM 的 AGL 锚点选择", "AGL anchor selection for touchdown FPM" },
    { "接地后固定160ms稳健G复算", "Fixed 160 ms robust-G recalculation after touchdown" },
    { "固定接地窗口垂直投影G", "Projected vertical G in fixed touchdown window" },
    { "固定160ms窗口不满足最少样本与跨度要求，回退第一次压缩全段P75。", "The fixed 160 ms window does not meet the minimum sample and span requirements; falling back to full first-compression P75." },
    { "G采样与最终分支复算", "G-sampling and final-branch recalculation" },
    { "样本有效: finalG=固定160ms P75（固定窗不足时回退全段P75）；样本无效: finalG=min(全段P75, FPM备用上限)。物理平均G和冲量闭合误差只用于复核。", "Valid samples: finalG=fixed 160 ms P75 (fall back to full-event P75 when the fixed window is insufficient); invalid samples: finalG=min(full-event P75, FPM fallback cap). Physics-derived average G and impulse-closure error are for review only." },
    { "0.25秒聚合轨迹表（高精度复算输入）", "0.25 s Aggregated Trajectory Table (High-Precision Recalculation Input)" },
    { "未知（代码 ", "Unknown (code " },
    { "触地点未落入任何跑道几何包络", "Touchdown point did not fall within any runway geometry envelope" },
    { "多条跑道候选过于接近，拒绝猜测", "Multiple runway candidates were too close; no guess was made" },
    { "所有 apt.dat 均未找到该机场", "The airport was not found in any apt.dat source" },
    { "没有可读取的 apt.dat 数据源", "No readable apt.dat source was available" },
    { "进近跑道预扫描超过时间预算", "Approach runway pre-scan exceeded its time budget" },
    { "跑道识别超过独立时间预算", "Runway identification exceeded its independent time budget" },
    { "跑道识别超过时间预算", "Runway identification exceeded its time budget" },
    { "apt.dat 解析异常", "apt.dat parsing error" },
    { "X-Plane 根目录不可用", "X-Plane root directory unavailable" },
    { "未找到可读取的 apt.dat", "No readable apt.dat source was found" },
    { "首次接地前拉平聚合点不足", "Insufficient flare buckets before first touchdown" },
    { "正在采集第一次起落架压缩", "Capturing the first landing-gear compression" },
    { "接地过程记录完成", "Touchdown-process recording complete" },
    { "正在记录接地过程", "Recording the touchdown process" },
    ...
    { "附录B、第二次触地数学复算", "Appendix B. Second-Touchdown Mathematical Recalculation" },
    ...
    { "三、操纵输入与舵面响应", "III. Control Inputs and Surface Response" },
    { "四、飞行、位置与环境参考", "IV. Flight, Position and Environment" },
    { "五、着陆数据复核", "V. Landing Data Review" },
    { "六、评分阈值与显示设置", "VI. Rating Thresholds and Display Settings" },
    ...
    { "附录A、第一次触地数学复算", "Appendix A. First-Touchdown Mathematical Recalculation" },
    ...
    { "相对风", "Relative wind" },
    { "完整接地过程记录时长", "Full touchdown-process trace duration" },
    { "接地后记录时长", "Post-touchdown trace duration" },
    { "首次接地 T+秒", "First touchdown T+s" },
    { "轨迹结束原因", "Trace end reason" },
    { "记录到的离地阶段数", "Recorded airborne episodes" },
    { "记录到的接触次数", "Recorded contact count" },
    { "首次接地后连续记录满2秒", "Recorded 2 continuous seconds after first touchdown" },
    { "最后一次接地后连续记录满2秒", "Recorded 2 continuous seconds after final touchdown" },
    { "达到接地过程8秒安全上限", "Reached the 8 s touchdown-process safety limit" },
    { "操纵输入与舵面响应", "Control Inputs and Surface Response" },
    { "操纵数据采集", "Control-data capture" },
    { "玩家输入来源", "Pilot-input source" },
    { "实际舵面来源", "Actual-surface source" },
    { "可用操纵通道", "Available input channels" },
    { "可用舵面通道", "Available surface channels" },
    { "操纵原始采样频率", "Raw control sampling rate" },
    { "操纵响应样本数", "Control-response sample count" },
    { "缺失通道说明", "Missing-channel note" },
    { "首次接地操纵快照", "First-touchdown control snapshot" },
    { "完整（9/9）", "Complete (9/9)" },
    { "部分（", "Partial (" },
    { "不可用（0/9）", "Unavailable (0/9)" },
    { "标准摇杆比例 DataRef", "standard joystick-ratio DataRefs" },
    { "标准 flightmodel2/wing 常规飞机数组映射", "standard flightmodel2/wing conventional-aircraft array mapping" },
    { "通道角度保留 X-Plane 原始符号；输入为 -1 至 +1，舵面为度；不计算跨单位差值或响应延迟。", "Surface angles retain X-Plane's raw signs; inputs are -1 to +1 and surfaces are degrees; no cross-unit difference or response-lag conclusion is calculated." },
    { "0.10秒操纵响应表", "0.10 s Control Response Table" },
    { "T+秒", "T+s" },
...
-- 先替换较长的完整短语，避免“跑道路面”等短词提前破坏
-- “跑道路面代码”“操纵原始采样频率”之类的复合字段。
table.sort(log_tools.report_translations, function(a, b)
...
function log_tools.control_value(value, valid, decimals)
    if not valid then return "NA" end
    return string.format("%." .. tostring(decimals or 6) .. "f", tonumber(value) or 0)
end

function log_tools.control_channel_list(kind)
    local channels = {}
    if kind == "input" then
        local ordered = { "pitch", "roll", "yaw" }
        local labels = { pitch = "Pitch", roll = "Roll", yaw = "Yaw" }
        for i = 1, #ordered do
            local key = ordered[i]
            if LMM_CONTROL_REFS.inputs[key].available then
                channels[#channels + 1] = labels[key]
            end
        end
    else
        local ordered = {
            "elevator_1", "elevator_2", "aileron_1",
            "aileron_2", "rudder_1", "rudder_2"
        }
        local labels = {
            elevator_1 = "Elevator1", elevator_2 = "Elevator2",
            aileron_1 = "Aileron1", aileron_2 = "Aileron2",
            rudder_1 = "Rudder1", rudder_2 = "Rudder2"
        }
        for i = 1, #ordered do
            local key = ordered[i]
            local spec = LMM_CONTROL_REFS.surfaces[key]
            local array_spec = LMM_CONTROL_REFS.arrays[spec.array_key]
            if array_spec ~= nil and array_spec.available then
                channels[#channels + 1] = labels[key]
            end
        end
    end
    return #channels > 0 and table.concat(channels, ", ")
        or log_tools.ui_text("无", "None")
end

function log_tools.engine_channel_list(kind)
    local channels = {}
    local count = LMM_CONTROL_REFS.recorded_engine_count
    local available = false
    if kind == "throttle" then
        available = LMM_CONTROL_REFS.engines.throttle.available
            or LMM_CONTROL_REFS.engines.throttle_jet_reverse.available
            or LMM_CONTROL_REFS.engines.throttle_beta_reverse.available
    elseif kind == "n1" then
        available = LMM_CONTROL_REFS.n1.selected ~= nil
            or LMM_CONTROL_REFS.engines.n1_cockpit.available
            or LMM_CONTROL_REFS.engines.n1_flightmodel.available
    else
        available = LMM_CONTROL_REFS.airbus.active
            and LMM_CONTROL_REFS.airbus.lever.available
    end
    if available then
        for i = 1, count do
            if kind == "throttle" then
                channels[#channels + 1] = "Throttle" .. tostring(i)
            elseif kind == "n1" then
                channels[#channels + 1] = "N1_" .. tostring(i)
            else
                channels[#channels + 1] = "Detent" .. tostring(i)
            end
        end
    end
    return #channels > 0 and table.concat(channels, ", ")
...
local function write_primary_math_audit(file)
    ...
    file:write(string.format(
        log_tools.ui_text(
            ...
            "Fallback P25 index = ceil(%d x %.2f) = %d; %.9f m/s x 196.850394 = %.9f fpm; algorithm fallback = %.9f fpm\n\n"
        ),
        physical_count,
        PHYSICAL_FPM_PERCENTILE,
        physical_index,
    ...
    file:write(string.format(
        log_tools.ui_text(
            ...
            "Touchdown-window median (n=%d) = %.9f m/s; converted = %.9f fpm; final physical candidate = %.9f fpm\n\n"
        ),
        contact_count,
        contact_value,
    ...
    file:write(string.format(
        log_tools.ui_text(
            ...
            "Touchdown-window physical samples=%d; VVI-reference samples=%d; geometric samples=%d; slope pairs=%d; span=%.9f s; physical candidate=%.9f; VVI candidate=%.9f; AGL anchor=%.9f fpm.\n"
        ),
    ...
    file:write(string.format(
        log_tools.ui_text(
            ...
            "Rule: fixed window from first contact, length=%.3f s, minimum samples=%d, minimum actual span=%.3f s; take P%.0f from the fixed window without sliding for a maximum.\n"
        ),
    ...
    if landing_analysis.local_event_g_valid then
        ...
        file:write(string.format(
            log_tools.ui_text(
                ...
                "Fixed window: T%+.9f to T%+.9f s; samples=%d; span=%.9f s; P75 index=%d; recalculated=%.9f G; stored by algorithm=%.9f G.\n"
            ),
    ...
    file:write(string.format(
        log_tools.ui_text(
            ...
            "Final robust G = fixed 160 ms P75 %.9f (full-event P75 reference %.9f) = %.9f G.\n\n"
        ),
        landing_analysis.local_event_g,
        landing_analysis.curve_g,
    ...
    file:write(string.format(
        log_tools.ui_text(
            ...
            "High-G diagnostic threshold = 1.0 + (peak-1.0) x 0.80 = %.9f G\n"
        ),
        high_threshold
    ))
    ...
    file:write(string.format(
        log_tools.ui_text(
            ...
            "Physics-derived average G = clamp(1,5, 1.0 + delta-v/(9.80665 x compression duration)) = clamp(1,5, 1.0 + %.9f/(9.80665 x %.9f)) = %.9f G\n\n"
        ),
        landing_analysis.velocity_delta_mps,
        landing_analysis.stop_duration_seconds,
        landing_analysis.equivalent_g
    ...
    file:write(string.format(
        log_tools.ui_text(
            "冲量闭合复核: <= %.0f%% 为通过，<= %.0f%% 为可复核，超过则提示偏差；复核结果不覆盖有效的实测稳健G。\n",
            "Impulse-closure review: <= %.0f%% passes, <= %.0f%% remains reviewable, and larger error is flagged; the review never overrides a valid measured robust G.\n"
        ),
        CONSISTENCY_HIGH_MAX_ERROR * 100,
        CONSISTENCY_MEDIUM_MAX_ERROR * 100
    ))
...
function log_tools.write_rollout_report(file)
    local rec=log_tools.recording
    local r=log_tools.dev.route
    local en=runtime_state.document_language=="en"
    local function line(cn,english,value) file:write((en and english or cn)..": "..tostring(value).."\n") end
    file:write(en and "\nRollout centreline grading\n" or "\n滑跑中心线考核\n")
    line("滑跑规则","Rollout rule","rollout-grade-1; GS >=60 kt")
    if not r or r.session_id~=landing_context.recording_id or r.samples==0 then
        line("滑跑评价","Rollout assessment",en and "Unavailable: no reliable runway / qualifying samples" or "未参与考核：无可靠跑道或有效高速滑跑样本")
        return
    end
    local complete=rec.id==landing_context.recording_id and rec.done and rec.phase=="COMPLETE"
    local phase=complete and "COMPLETE" or rec.done and "INCOMPLETE" or "IN_PROGRESS"
    line("滑跑考核状态","Rollout grading phase",phase)
    line("滑跑评价","Rollout assessment",r:description(en))
    line("Rollout assessment (EN)","Rollout assessment (EN)",r:description(true))
    line("滑跑已触发降级","Rollout penalty triggered",r.revision>0 and 1 or 0)
    line("滑跑评分等级","Rollout rating",landing_status)
    line("滑跑最大偏差","Rollout maximum offset",string.format("%.2f m",r.max_offset))
    line("滑跑最大横向跨度","Rollout maximum lateral span",string.format("%.2f m",r.max_span))
    line("滑跑连续未回3米内最长时间","Rollout longest outside 3 m",string.format("%.2f s",r.longest_outside))
    line("滑跑9米触发后未回7米内最长时间","Rollout longest unrecovered 9 m event",string.format("%.2f s",r.longest_red))
    line("滑跑有效样本","Rollout qualifying samples",r.samples)
    line("滑跑前段缺失","Rollout front missing",rec.rollout_front_missing and 1 or 0)
    for _,e in ipairs(r.evidence) do
        line("滑跑触发证据","Rollout trigger evidence",string.format("%s; T%+.2f s; %.2f m; %.2f s; GS %.1f kt",e.kind,e.t-(rec.touch_time or e.t),e.value,e.duration,e.gs))
    end
    file:write(en and "Each rule applies once. Non-red steps stop at ATTENTION. An early snapshot includes only evidence available at that time.\n\n"
        or "每项仅触发一次，普通降级最多至黄色；早期快照仅包含当时已有证据，最终完整报告随滑跑更新。\n\n")
end

function log_tools.build_landing_log_payload(existing_path)
    -- 先把报告组装到内存。实际文件写入由 landing_jobs 在后续帧分块完成。
    -- Source choices are already captured per sample; report generation never changes them.
    LMM_CONTROL_REFS.resolve_input_sources(flare_trace.slots, flare_trace.count)
    local log_path = existing_path or next_log_file_path()
    local output_chunks = {}
    local raw_file = {}
    function raw_file:write(...)
        for i = 1, select("#", ...) do
            local value = select(i, ...)
            if value ~= nil then
                output_chunks[#output_chunks + 1] = tostring(value)
            end
        end
        return self
    end
    function raw_file:close()
        return true
    end
    ...
    raw_file:write(log_tools.ui_text(
        "100 ft 节选（完整记录见末尾）\n接地阶段快照；滑跑可能仍在录制。\n\n",
        "100 ft excerpt - Full recording at the end of this report.\nTouchdown snapshot; rollout may still be recording.\n\n"))

    -- Selected physical runway ends (true coordinates), independent of taxiway geometry.
    local route=log_tools.dev.route
    if route and route.session_id==landing_context.recording_id then
        local r=route.runway
        raw_file:write(string.format("LMM_RUNWAY\t1\t%.10f\t%.10f\t%.10f\t%.10f\t%.3f\t%.10f\t%.10f\n",
            route.reverse and r.lat2 or r.lat1,route.reverse and r.lon2 or r.lon1,
            route.reverse and r.lat1 or r.lat2,route.reverse and r.lon1 or r.lon2,r.width_m,
            landing_context.touch_latitude,landing_context.touch_longitude))
    end
    ...
    file:write(string.format(
        "首次接地 T+秒: %.2f s\n",
        math.max(0, flare_trace.touch_time - flare_trace.start_time)
    ))
    file:write(string.format(
        "完整接地过程记录时长: %.2f s\n",
        flare_analysis.trace_duration_seconds
    ))
    file:write(string.format(
        "接地后记录时长: %.2f s\n",
        flare_analysis.post_touch_duration_seconds
    ))
    file:write("轨迹结束原因: " .. flare_trace.end_reason .. "\n")
    file:write("记录到的离地阶段数: " .. tostring(flare_trace.airborne_episode_count) .. "\n")
    file:write("记录到的接触次数: " .. tostring(flare_trace.touch_count) .. "\n")
    ...
    if landing_analysis.math_log_enabled then
        ...
        file:write("T+秒         RA(ft)       轨迹FPM       VVI          IAS        GS         Pitch       AoA         Roll        物理FPM       G            Ground\n")
        file:write("------------------------------------------------------------------------------------------------------------------------------------------------\n")
        for i = 1, flare_trace.bucket_count do
            local bucket = flare_trace.buckets[i]
            file:write(string.format(
                "%11.9f  %11.6f  %+13.9f  %+11.6f  %10.6f  %10.6f  %+11.6f  %+10.6f  %+10.6f  %+13.9f  %11.9f  %.6f\n",
        ...
        for i = 3, flare_trace.touch_bucket_index do
            local p1 = flare_trace.buckets[i - 2]
            local p2 = flare_trace.buckets[i - 1]
            local p3 = flare_trace.buckets[i]
            local dt1 = p2.t - p1.t
        ...
        for i = 1, flare_trace.bucket_count do
            local bucket = flare_trace.buckets[i]
            file:write(string.format(
                "%5.1f  %7.1f  %8.0f  %5.0f  %4.0f  %4.0f  %+6.1f  %+5.1f  %+5.1f  %+8.0f  %4.2f  %.2f\n",
                bucket.t - flare_trace.start_time,
                bucket.agl_ft,
                bucket.selected_fpm,
                bucket.vvi_fpm,
    ...
    file:write("三、操纵输入与舵面响应\n")
    file:write("----------------------------------------------------------------------\n")
    local input_available_count = 0
    for _, spec in pairs(LMM_CONTROL_REFS.inputs) do
        if spec.available then input_available_count = input_available_count + 1 end
    end
    local control_available_count = LMM_CONTROL_REFS.available_count()
    local surface_available_count = control_available_count - input_available_count
    local capture_status
    if control_available_count >= 9 then
        capture_status = log_tools.ui_text("完整（9/9）", "Complete (9/9)")
    elseif control_available_count > 0 then
        capture_status = log_tools.ui_text(
            "部分（" .. tostring(control_available_count) .. "/9）",
            "Partial (" .. tostring(control_available_count) .. "/9)"
        )
    else
        capture_status = log_tools.ui_text("不可用（0/9）", "Unavailable (0/9)")
    end
    file:write("操纵数据采集: " .. capture_status .. "\n")
    file:write("玩家输入来源: " .. (input_available_count > 0
        and LMM_CONTROL_REFS.input_source_text(runtime_state.document_language == "en")
        or log_tools.ui_text("不可用", "Unavailable")) .. "\n")
    file:write("实际舵面来源: " .. (surface_available_count > 0
        and log_tools.ui_text(
            "标准 flightmodel2/wing 常规飞机数组映射",
            "standard flightmodel2/wing conventional-aircraft array mapping"
        )
        or log_tools.ui_text("不可用", "Unavailable")) .. "\n")
    file:write("可用操纵通道: " .. log_tools.control_channel_list("input") .. "\n")
    file:write("可用舵面通道: " .. log_tools.control_channel_list("surface") .. "\n")
    file:write(log_tools.ui_text("发动机数量: ", "Engine count: ")
        .. tostring(LMM_CONTROL_REFS.engine_count) .. "\n")
    file:write(log_tools.ui_text("动力通道记录上限: ", "Recorded engine-channel limit: ")
        .. tostring(LMM_CONTROL_REFS.max_recorded_engines)
        .. log_tools.ui_text(" 台", " engines") .. "\n")
    file:write(log_tools.ui_text("油门参考: ", "Throttle reference: ")
        .. (LMM_CONTROL_REFS.recorded_engine_count > 0
            and log_tools.ui_text(
                "标准 cockpit2 逐台油门手柄；按发动机类型选择喷气反推或桨叶 Beta/反推范围",
                "standard per-engine cockpit2 throttle handles; jet reverse or propeller beta/reverse range is selected by engine type"
            )
            or log_tools.ui_text("不可用", "Unavailable")) .. "\n")
    file:write(log_tools.ui_text("油门杆类型: ", "Throttle lever type: ")
        .. (LMM_CONTROL_REFS.airbus.family and "AIRBUS_DETENT" or "CONVENTIONAL")
        .. "\n")
    file:write(log_tools.ui_text("N1 来源: ", "N1 source: ")
        .. LMM_CONTROL_REFS.n1_source_text(runtime_state.document_language == "en")
        .. "\n")
    file:write(log_tools.ui_text("空客卡位来源: ", "Airbus detent source: ")
        .. (LMM_CONTROL_REFS.airbus.active and LMM_CONTROL_REFS.airbus.lever.available
            and "ToLiss AirbusFBW/throttle_input + AirbusFBW/THRLeverMode"
            or log_tools.ui_text("当前机型无已验证专用接口，记录为 NA", "No verified aircraft-specific interface for the current aircraft; recorded as NA"))
        .. "\n")
    file:write(log_tools.ui_text("可用油门通道: ", "Available throttle channels: ")
        .. log_tools.engine_channel_list("throttle") .. "\n")
    file:write(log_tools.ui_text("可用 N1 通道: ", "Available N1 channels: ")
        .. log_tools.engine_channel_list("n1") .. "\n")
    file:write(log_tools.ui_text("可用空客卡位通道: ", "Available Airbus detent channels: ")
        .. log_tools.engine_channel_list("detent") .. "\n")
    file:write(string.format(
        "操纵原始采样频率: %.1f Hz\n",
        1 / FLARE_CONFIG.sample_interval_seconds
    ))
    file:write("操纵响应样本数: " .. tostring(flare_trace.control_sample_count) .. "\n")
    file:write("缺失通道说明: " .. (#LMM_CONTROL_REFS.missing > 0
        and table.concat(LMM_CONTROL_REFS.missing, ", ")
        or log_tools.ui_text("无", "None")) .. "\n")
    file:write(log_tools.ui_text(
        "通道角度保留 X-Plane 原始符号；输入为 -1 至 +1，舵面为度；N1 优先使用已验证的机模仪表接口，其次回退到 cockpit2 和 flightmodel；N1 列始终保留，非 N1 发动机可能为 0；不计算跨单位差值或响应延迟。",
        "Surface angles retain X-Plane's raw signs; inputs are -1 to +1 and surfaces are degrees; N1 prefers a verified aircraft indication interface, then falls back to cockpit2 and flightmodel; N1 columns are always retained and may read 0 for non-N1 engines; no cross-unit difference or response-lag conclusion is calculated."
    ) .. "\n")

    local touch_slot = flare_trace.slots[flare_trace.touch_sample_index]
    if touch_slot ~= nil and flare_trace.touch_sample_index > 0 then
        file:write("首次接地操纵快照: "
            .. "PitchIn=" .. log_tools.control_value(touch_slot.pitch_input_ratio, touch_slot.pitch_input_valid, 6)
            .. ", RollIn=" .. log_tools.control_value(touch_slot.roll_input_ratio, touch_slot.roll_input_valid, 6)
            .. ", YawIn=" .. log_tools.control_value(touch_slot.yaw_input_ratio, touch_slot.yaw_input_valid, 6)
            .. ", Elevator1=" .. log_tools.control_value(touch_slot.elevator_1_deg, touch_slot.elevator_1_valid, 6)
            .. ", Elevator2=" .. log_tools.control_value(touch_slot.elevator_2_deg, touch_slot.elevator_2_valid, 6)
            .. ", Aileron1=" .. log_tools.control_value(touch_slot.aileron_1_deg, touch_slot.aileron_1_valid, 6)
            .. ", Aileron2=" .. log_tools.control_value(touch_slot.aileron_2_deg, touch_slot.aileron_2_valid, 6)
            .. ", Rudder1=" .. log_tools.control_value(touch_slot.rudder_1_deg, touch_slot.rudder_1_valid, 6)
            .. ", Rudder2=" .. log_tools.control_value(touch_slot.rudder_2_deg, touch_slot.rudder_2_valid, 6)
            .. ", Throttle1=" .. log_tools.control_value(touch_slot.throttle_1_ratio, touch_slot.throttle_1_valid, 6)
            .. ", Throttle2=" .. log_tools.control_value(touch_slot.throttle_2_ratio, touch_slot.throttle_2_valid, 6)
            .. ", Throttle3=" .. log_tools.control_value(touch_slot.throttle_3_ratio, touch_slot.throttle_3_valid, 6)
            .. ", Throttle4=" .. log_tools.control_value(touch_slot.throttle_4_ratio, touch_slot.throttle_4_valid, 6)
            .. ", N1_1=" .. log_tools.control_value(touch_slot.n1_1_percent, touch_slot.n1_1_valid, 4)
            .. ", N1_2=" .. log_tools.control_value(touch_slot.n1_2_percent, touch_slot.n1_2_valid, 4)
            .. ", N1_3=" .. log_tools.control_value(touch_slot.n1_3_percent, touch_slot.n1_3_valid, 4)
            .. ", N1_4=" .. log_tools.control_value(touch_slot.n1_4_percent, touch_slot.n1_4_valid, 4)
            .. ", Detent1=" .. tostring(touch_slot.airbus_detent_1 or "NA")
            .. ", Detent2=" .. tostring(touch_slot.airbus_detent_2 or "NA")
            .. ", Detent3=" .. tostring(touch_slot.airbus_detent_3 or "NA")
            .. ", Detent4=" .. tostring(touch_slot.airbus_detent_4 or "NA")
            .. ", DetentMode=" .. log_tools.control_value(touch_slot.airbus_mode, touch_slot.airbus_mode_valid, 0)
            .. "\n")
    else
        file:write("首次接地操纵快照: " .. log_tools.ui_text("不可用", "Unavailable") .. "\n")
    end

    file:write("\n0.10秒操纵响应表\n")
    file:write("T+秒  PitchIn  RollIn  YawIn  Elevator1  Elevator2  Aileron1  Aileron2  Rudder1  Rudder2  Throttle1  Throttle2  Throttle3  Throttle4  N1_1  N1_2  N1_3  N1_4  Detent1  Detent2  Detent3  Detent4  DetentMode\n")
    file:write("----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n")
    for i = 1, flare_trace.count do
        local sample = flare_trace.slots[i]
        if sample.t >= flare_trace.start_time then
            local time_decimals = landing_analysis.math_log_enabled and 9 or 3
            local value_decimals = landing_analysis.math_log_enabled and 9 or 5
            file:write(
                log_tools.control_value(sample.t - flare_trace.start_time, true, time_decimals) .. "  "
                .. log_tools.control_value(sample.pitch_input_ratio, sample.pitch_input_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.roll_input_ratio, sample.roll_input_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.yaw_input_ratio, sample.yaw_input_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.elevator_1_deg, sample.elevator_1_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.elevator_2_deg, sample.elevator_2_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.aileron_1_deg, sample.aileron_1_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.aileron_2_deg, sample.aileron_2_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.rudder_1_deg, sample.rudder_1_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.rudder_2_deg, sample.rudder_2_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.throttle_1_ratio, sample.throttle_1_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.throttle_2_ratio, sample.throttle_2_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.throttle_3_ratio, sample.throttle_3_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.throttle_4_ratio, sample.throttle_4_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.n1_1_percent, sample.n1_1_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.n1_2_percent, sample.n1_2_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.n1_3_percent, sample.n1_3_valid, value_decimals) .. "  "
                .. log_tools.control_value(sample.n1_4_percent, sample.n1_4_valid, value_decimals) .. "  "
                .. tostring(sample.airbus_detent_1 or "NA") .. "  "
                .. tostring(sample.airbus_detent_2 or "NA") .. "  "
                .. tostring(sample.airbus_detent_3 or "NA") .. "  "
                .. tostring(sample.airbus_detent_4 or "NA") .. "  "
                .. log_tools.control_value(sample.airbus_mode, sample.airbus_mode_valid, 0)
                .. "\n"
            )
        end
    end
    ...
    file:write(string.format(
        log_tools.ui_text(
            "位置微调: X %+d px / Y %+d px\n",
            "Fine position offset: X %+d px / Y %+d px\n"
        ),
        runtime_state.popup_offset_x or 0,
        runtime_state.popup_offset_y or 0
    ))
    ...
    return true, {
        path = log_path,
        recording_id = landing_context.recording_id,
        content = table.concat(output_chunks),
        build_ms = 0
    }
end

function log_tools.finish_landing_log_write(now, state)
    local close_ok, close_error = state.file:close()
    state.file = nil
    if close_ok == nil then
        return false, tostring(close_error)
    end

    if state.recording_id==log_tools.recording.id then
        log_tools.recording:attach_summary(state.content,state.path)
    end
    local file_name = file_name_from_path(state.path)
    -- 新日志直接追加到内存索引，避免每次落地后重新遍历整个文件夹。
    log_tools.register_log_filename(file_name)
    landing_report_notice.file_name = file_name
    landing_report_notice.path = state.path
    landing_report_notice.text = log_tools.popup_text(
        "落地报告已生成",
        "Landing report generated"
    )
    landing_report_notice.ready = POPUP_MODE ~= "off"
        and POPUP_MODE ~= "clean"
        and runtime_state.replay_active == false
    landing_report_notice.flash_started_at = 0
    landing_report_notice.flash_until_time = 0
    landing_report_notice.debug_until_time = 0
    if DEBUG_MODE and POPUP_MODE ~= "clean" and runtime_state.replay_active == false then
        landing_report_notice.debug_until_time = now + REPORT_NOTICE_SECONDS
    end
    if landing_report_notice.ready
        and (POPUP_MODE == "immediate" or runtime_state.deferred_popup_done) then
        log_tools.start_report_flash(now)
        show_until = math.max(show_until, landing_report_notice.flash_until_time)
    end
    ...
    if logMsg then
        logMsg(string.format(
            "[StarLux LMM] Landing log saved incrementally: %s | %d bytes | build %.2f ms | write %.2f ms",
            state.path,
            state.length,
            state.build_ms,
            (os.clock() - state.write_started_clock) * 1000
        ))
    end
    return true
end

function log_tools.fail_landing_log_write(state, error_text)
    if state ~= nil and state.file ~= nil then
        pcall(function() state.file:close() end)
        state.file = nil
    end
    if logMsg then
        logMsg("[StarLux LMM] Landing log failed, but flight monitoring will continue: "
            .. tostring(error_text))
    end
end

function log_tools.process_incremental_landing_log_write(now)
    local state = landing_jobs.log_write
    if state == nil then return end

    if state.file == nil then
        local file, open_error = io.open(state.path, "wb")
        if file == nil then
            log_tools.fail_landing_log_write(state, open_error)
            landing_jobs.log_write = nil
            return
        end
        state.file = file
        state.write_started_clock = os.clock()
    end

    local chunk_end = math.min(
        state.length,
        state.offset + landing_jobs.write_chunk_bytes - 1
    )
    local write_ok, write_error = state.file:write(
        string.sub(state.content, state.offset, chunk_end)
    )
    if write_ok == nil then
        log_tools.fail_landing_log_write(state, write_error)
        landing_jobs.log_write = nil
        return
    end
    state.offset = chunk_end + 1

    if state.offset > state.length then
        local finish_ok, finish_error = log_tools.finish_landing_log_write(now, state)
        if not finish_ok then
            log_tools.fail_landing_log_write(state, finish_error)
        end
        landing_jobs.log_write = nil
    end
end

function log_tools.begin_incremental_landing_log_write()
    log_landing_summary()
    local build_started = os.clock()
    local call_ok, build_ok, result = pcall(log_tools.build_landing_log_payload)
    if not call_ok or not build_ok then
        local error_text = not call_ok and build_ok or result
        log_tools.fail_landing_log_write(nil, error_text)
        return false
    end

    result.build_ms = (os.clock() - build_started) * 1000
    result.offset = 1
    result.length = string.len(result.content)
    result.file = nil
    result.write_started_clock = 0
    landing_jobs.log_write = result
    return true
end
...
local function process_landing_jobs(now, allow_prefetch_scan)
    ...
    -- 每帧最多写入一个固定小块；完成提示只在文件关闭成功后显示。
    log_tools.process_incremental_landing_log_write(now)
    ...
    if landing_jobs.log_pending == true and now >= landing_jobs.log_after then
        ...
        log_tools.begin_incremental_landing_log_write()
    end
end

local function begin_bounce_monitor(now, radio_alt_ft)
    bounce_state.monitoring = true
    bounce_state.detected = false
    bounce_state.phase = "waiting"
    bounce_state.first_touch_time = now
    bounce_state.recording_id = not log_tools.recording.done and log_tools.recording.id or nil
    bounce_state.last_time = now
    bounce_state.low_speed_since = nil
    bounce_state.climb_since = nil
    bounce_state.first_touch_height = radio_alt_ft or 0
    bounce_state.airborne_start_time = 0
    bounce_state.airborne_duration_seconds = 0
    bounce_state.airborne_peak_agl_ft = 0
    bounce_state.airborne_peak_upward_mps = 0
...
local function apply_bounce_score(now)
    ...
    if bounce_state.recording_id and bounce_state.recording_id==log_tools.recording.id then
        log_tools.recording.summary_dirty=true
    end
    refresh_popup_cache()
...
local function finish_second_touch_analysis(now)
    ...
    -- Keep this landing armed against accidental re-arming during further
    -- bounces. The first confirmed second-touch analysis remains the audit.
    bounce_state.phase = "confirmed"
    apply_bounce_score(now)
    refresh_popup_cache()
end

local function process_bounce_monitor(now, on_ground, radio_alt_ft, local_vy_mps, current_g, gs_kts)
    if not bounce_state.monitoring then return end
    local rec=log_tools.recording
    if rec.pause_at then bounce_state.last_time=now; return end
    local interrupted=bounce_state.last_time and (now<bounce_state.last_time or now-bounce_state.last_time>1)
    bounce_state.last_time=now
    if bounce_state.recording_id and (rec.id~=bounce_state.recording_id or rec.done) then interrupted=true end
    -- Also works when streaming is unavailable. Stop at taxi speed or a
    -- sustained climb well above the touchdown, never at a short time limit.
    if on_ground==1 and gs_kts and gs_kts<30 then
        bounce_state.low_speed_since=bounce_state.low_speed_since or now
    else bounce_state.low_speed_since=nil end
    if on_ground==0 and radio_alt_ft>bounce_state.first_touch_height+300 and local_vy_mps>3 then
        bounce_state.climb_since=bounce_state.climb_since or now
    else bounce_state.climb_since=nil end
    if interrupted or (bounce_state.low_speed_since and now-bounce_state.low_speed_since>=2)
        or (bounce_state.climb_since and now-bounce_state.climb_since>5) then
        bounce_state.monitoring=false; bounce_state.phase="complete"; return
    end
    ...
    elseif bounce_state.phase == "airborne" then
        ...
        elseif on_ground == 1 and was_on_ground == 0 then
            ...
            local valid_bounce = bounce_state.airborne_duration_seconds
                ...
                and (
                    bounce_state.airborne_peak_agl_ft-bounce_state.first_touch_height >= BOUNCE_CONFIG.min_peak_agl_ft
...
local function process_landing_analysis(now, local_vy_mps)
    ...
    elseif landing_analysis.phase == "wait_flare_trace" then
        if flare_trace.complete == true or flare_trace.active == false then
...
function ma_open_settings_window()
    if log_tools.open_native_dialog("settings", function(gui) ma_build_settings_window(nil,nil,nil,gui) end,
        log_tools.settings_text("StarLux LMM - 设置", "StarLux LMM - Settings"), 860, 850) then return end
    ...
    local window_w = 640
    local window_h = math.max(480, math.min(900, screen_h - 80))
    settings_window = float_wnd_create(window_w, window_h, 1, true)
    ...
    float_wnd_set_position(
        settings_window,
        math.floor((screen_w - window_w) / 2),
        math.floor((screen_h - window_h) / 2)
    )
end

function log_tools.dialog_section(gui, chinese, english)
    local title=log_tools.settings_text(chinese,english)
    if gui.Section then gui.Section(title) else gui.Separator(); gui.TextUnformatted(title) end
end

function log_tools.dialog_notice(gui, text, tone)
    if gui.Notice then gui.Notice(text,tone) else gui.TextUnformatted(text) end
end
...
function ma_build_settings_window(wnd, x, y, gui)
    ...
    if imgui.Tabs then
        log_tools.settings_ui_tab=imgui.Tabs({
            {"general",log_tools.settings_text("通用", "General")},
            {"position",log_tools.settings_text("位置", "Position")},
            {"colors",log_tools.settings_text("配色", "Colours")},
            {"fonts",log_tools.settings_text("字体", "Fonts")},
            {"tools",log_tools.settings_text("工具", "Tools")},
            {"live",log_tools.settings_text("实时视图", "Live view")}
        },log_tools.settings_ui_tab or "general")
        page=log_tools.settings_ui_tab
    end
    ...
    if imgui.RadioButton("English##lmm_language_en", runtime_state.document_language == "en") then
        log_tools.set_global_language("en")
    end
    log_tools.dialog_section(imgui,"落地数据弹窗时机", "Landing data popup timing")
    ...
    log_tools.dialog_section(imgui,"精准跑道与触地点", "Runway and touchdown point")
    local runway_changed, new_runway_value = imgui.Checkbox(log_tools.settings_text("启用精准跑道识别", "Enable precise runway identification") .. "##lmm_runway", runtime_state.runway_detection_enabled)
    ...
    log_tools.dialog_section(imgui,"弹窗显示时长", "Popup duration")
    ...
    if page=="all" or page=="position" then
    log_tools.dialog_section(imgui,"弹窗位置", "Popup position")
    if imgui.BeginCombo(log_tools.settings_text("屏幕位置", "Screen position") .. "##lmm_position", position_label(POPUP_POSITION)) then
        for i = 1, #POSITION_OPTIONS do
            ...
            local label = position_label(option.id)
            if imgui.Selectable(label, POPUP_POSITION == option.id) then POPUP_POSITION = option.id; log_tools.dev.config.popup_free_x=-1; log_tools.dev.config.popup_free_y=-1; save_settings() end
        end
        imgui.EndCombo()
    end
    imgui.TextUnformatted(log_tools.settings_text("位置微调 / 每次 5 像素", "Fine position / 5 px per click"))
    if imgui.Button(log_tools.settings_text("上移", "Up") .. "##lmm_offset_up", 72, 26) then
        runtime_state.popup_offset_y = math.min(1000, (runtime_state.popup_offset_y or 0) + 5)
        save_settings()
    end
    imgui.SameLine()
    if imgui.Button(log_tools.settings_text("下移", "Down") .. "##lmm_offset_down", 72, 26) then
        runtime_state.popup_offset_y = math.max(-1000, (runtime_state.popup_offset_y or 0) - 5)
        save_settings()
    end
    imgui.SameLine()
    if imgui.Button(log_tools.settings_text("左移", "Left") .. "##lmm_offset_left", 72, 26) then
        runtime_state.popup_offset_x = math.max(-1000, (runtime_state.popup_offset_x or 0) - 5)
        save_settings()
    end
    imgui.SameLine()
    if imgui.Button(log_tools.settings_text("右移", "Right") .. "##lmm_offset_right", 72, 26) then
        runtime_state.popup_offset_x = math.min(1000, (runtime_state.popup_offset_x or 0) + 5)
        save_settings()
    end
    imgui.SameLine()
    if imgui.Button(log_tools.settings_text("复位", "Reset") .. "##lmm_offset_reset", 72, 26) then
        runtime_state.popup_offset_x = 0
        runtime_state.popup_offset_y = 0
        log_tools.dev.config.popup_free_x=-1; log_tools.dev.config.popup_free_y=-1
        save_settings()
    end
    imgui.TextUnformatted(string.format(
        log_tools.settings_text("当前微调：X %+d / Y %+d 像素", "Current fine offset: X %+d / Y %+d px"),
        runtime_state.popup_offset_x or 0,
        runtime_state.popup_offset_y or 0
    ))

    log_tools.dialog_section(imgui,"弹窗布局", "Popup layout")
    ...
    if page=="all" or page=="colors" then
    log_tools.dialog_section(imgui,"背景透明度", "Background opacity")
    ...
    log_tools.dialog_section(imgui,"各等级弹窗颜色", "Popup colour by rating")
    if imgui.RadioButton(log_tools.settings_text("轻柔", "Nice") .. "##lmm_colour_nice", log_tools.popup_style.edit_status == "NICE") then log_tools.popup_style.edit_status = "NICE" end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("稳定", "Stable") .. "##lmm_colour_stable", log_tools.popup_style.edit_status == "STABLE") then log_tools.popup_style.edit_status = "STABLE" end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("需注意", "Attention") .. "##lmm_colour_attention", log_tools.popup_style.edit_status == "ATTENTION") then log_tools.popup_style.edit_status = "ATTENTION" end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("不良落地", "UNSTABLE") .. "##lmm_colour_unstable", log_tools.popup_style.edit_status == "UNSTABLE") then log_tools.popup_style.edit_status = "UNSTABLE" end

    local edit_color = log_tools.popup_style.colors[log_tools.popup_style.edit_status]
    local red_changed, new_red = imgui.SliderFloat(log_tools.settings_text("红", "Red") .. "##lmm_popup_red", edit_color[1], 0, 1, "%.2f")
    local green_changed, new_green = imgui.SliderFloat(log_tools.settings_text("绿", "Green") .. "##lmm_popup_green", edit_color[2], 0, 1, "%.2f")
    local blue_changed, new_blue = imgui.SliderFloat(log_tools.settings_text("蓝", "Blue") .. "##lmm_popup_blue", edit_color[3], 0, 1, "%.2f")
    if red_changed or green_changed or blue_changed then
        edit_color[1] = log_tools.clamp_color(new_red)
        edit_color[2] = log_tools.clamp_color(new_green)
        edit_color[3] = log_tools.clamp_color(new_blue)
        save_settings()
    end
    local preview_red, preview_green, preview_blue = status_color(log_tools.popup_style.edit_status)
    local preview_text_red, preview_text_green, preview_text_blue = log_tools.popup_text_color(
        preview_red,
        preview_green,
        preview_blue
    )
    imgui.PushStyleColor(
        ...
        log_tools.imgui_rgba(preview_red, preview_green, preview_blue, 1)
    )
    imgui.PushStyleColor(
        imgui.constant.Col.Text,
        log_tools.imgui_rgba(preview_text_red, preview_text_green, preview_text_blue, 1)
    )
    imgui.Button(
        string.format("%s RGB %.2f / %.2f / %.2f##lmm_colour_preview", log_tools.popup_status_text(log_tools.popup_style.edit_status), preview_red, preview_green, preview_blue),
        330,
        30
    )
    imgui.PopStyleColor()
    imgui.PopStyleColor()
    imgui.SameLine()
    if imgui.Button(log_tools.settings_text("恢复此等级默认颜色", "Reset selected colour") .. "##lmm_colour_reset", 190, 30) then
        local default_color = log_tools.popup_style.defaults[log_tools.popup_style.edit_status]
        edit_color[1] = default_color[1]
        edit_color[2] = default_color[2]
        edit_color[3] = default_color[3]
        save_settings()
    end

    log_tools.dialog_section(imgui,"字体颜色", "Popup text colour")
    if imgui.RadioButton(log_tools.settings_text("自动高对比反色", "Automatic high contrast") .. "##lmm_text_contrast", log_tools.popup_style.text_color_mode == "contrast") then log_tools.popup_style.text_color_mode = "contrast"; save_settings() end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("固定白色", "Fixed white") .. "##lmm_text_white", log_tools.popup_style.text_color_mode == "white") then log_tools.popup_style.text_color_mode = "white"; save_settings() end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("固定黑色", "Fixed black") .. "##lmm_text_black", log_tools.popup_style.text_color_mode == "black") then log_tools.popup_style.text_color_mode = "black"; save_settings() end

    end
    if page=="all" or page=="fonts" then
    log_tools.dialog_section(imgui,"弹窗绘制方式", "Popup renderer")
    if imgui.RadioButton(log_tools.settings_text("自动（SDK 440）", "Automatic (SDK 440)") .. "##lmm_native_auto", log_tools.popup_style.renderer == "auto") then
        log_tools.popup_style.renderer = "auto"; save_settings()
    end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("兼容模式", "Compatibility") .. "##lmm_native_legacy", log_tools.popup_style.renderer == "legacy") then
        log_tools.popup_style.renderer = "legacy"; refresh_popup_cache(); save_settings()
    end
    ...
    imgui.TextUnformatted(log_tools.settings_text("原生字体未启用，已回退兼容显示。", "Native fonts unavailable; compatibility display is active."))
    if DEBUG_MODE then imgui.TextUnformatted(log_tools.settings_text("诊断详情：", "Diagnostic details: ") .. (log_tools.native_ui.error or "")) end
    log_tools.dialog_section(imgui,"字体大小", "Font size")
    if imgui.RadioButton(log_tools.settings_text("小号 10", "Small 10") .. "##lmm_size_small", log_tools.popup_style.font_size == "small") then log_tools.popup_style.font_size = "small"; refresh_popup_cache(); save_settings() end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("标准", "Normal") .. "##lmm_size_normal", log_tools.popup_style.font_size == "normal") then log_tools.popup_style.font_size = "normal"; refresh_popup_cache(); save_settings() end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("大号 18", "Large 18") .. "##lmm_size_large", log_tools.popup_style.font_size == "large") then log_tools.popup_style.font_size = "large"; refresh_popup_cache(); save_settings() end
    imgui.TextUnformatted(log_tools.settings_text("兼容模式：中文使用标准字号。", "Compatibility: standard size for Chinese."))
    end

    log_tools.dialog_section(imgui,"字体粗细", "Font weight")
    if imgui.RadioButton(log_tools.settings_text("常规", "Normal") .. "##lmm_weight_normal", log_tools.popup_style.font_weight == "normal") then log_tools.popup_style.font_weight = "normal"; save_settings() end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("中等", "Medium") .. "##lmm_weight_medium", log_tools.popup_style.font_weight == "medium") then log_tools.popup_style.font_weight = "medium"; save_settings() end
    imgui.SameLine()
    if imgui.RadioButton(log_tools.settings_text("加粗", "Bold") .. "##lmm_weight_bold", log_tools.popup_style.font_weight == "bold") then log_tools.popup_style.font_weight = "bold"; save_settings() end

    end
    if page=="all" or page=="tools" then
    log_tools.dialog_section(imgui,"调试", "Debug")
    local changed, new_debug_value = imgui.Checkbox(log_tools.settings_text("显示测量与扫描调试详情", "Show measurement and scan debug details") .. "##lmm_debug", DEBUG_MODE)
    if changed then DEBUG_MODE = new_debug_value; save_settings() end

    log_tools.dialog_section(imgui,"TXT 报告详细程度", "TXT report detail")
    ...
    if page=="all" or page=="live" then
        log_tools.dev:settings(imgui,log_tools.settings_text,DEBUG_MODE,save_settings)
    end
    log_tools.dialog_section(imgui,"预览与保存", "Preview and save")
    ...
    log_tools.dialog_notice(imgui,settings_save_ok and log_tools.settings_text("设置已自动保存。", "Settings are saved automatically.") or log_tools.settings_text("警告：设置保存失败，请检查 FlyWithLua Log.txt。", "Warning: settings could not be saved. Check FlyWithLua Log.txt."), settings_save_ok and "success" or "error")
end
...
function ma_open_log_manager()
    if log_tools.open_native_dialog("records", function(gui) ma_build_log_manager_window(nil,nil,nil,gui) end,
        log_tools.settings_text("StarLux LMM - 落地记录", "StarLux LMM - Landing records"), 900, 820) then return end
    if not SUPPORTS_FLOATING_WINDOWS then
        if logMsg then
            logMsg("[StarLux LMM] This FlyWithLua version does not support floating windows.")
        end
    ...
    float_wnd_set_title(log_manager_window, log_tools.settings_text("StarLux LMM - 落地记录", "StarLux LMM - Landing records"))
...
function ma_build_log_manager_window(wnd, x, y, gui)
    local imgui = gui or _G.imgui
    local record_count = #log_manager_state.records
    local page_count = math.max(1, math.ceil(record_count / LOG_MANAGER_PAGE_SIZE))
    if log_manager_state.page > page_count then log_manager_state.page = page_count end

    log_tools.dialog_section(imgui,"落地记录", "Landing records")
    imgui.TextUnformatted(
        string.format(
            log_tools.settings_text("共 %d 条记录", "%d landing records"),
            record_count
        )
    )
    ...
    imgui.SameLine()
    if imgui.Button(log_tools.settings_text("打开日志文件夹", "Open log folder") .. "##lmm_folder", 180, 26) then
    ...
    if log_manager_state.scan_error ~= "" then
        log_tools.dialog_notice(imgui,log_manager_state.scan_error,"error")
    end
    log_tools.dialog_section(imgui,"选择记录 / 打开报告", "Select a record / Open report")
    ...
    if record_count == 0 then
        ...
        for i = first_index, last_index do
            local record = log_manager_state.records[i]
            local open_record
            if imgui.Record then open_record=imgui.Record(record.display_name,"lmm_record_"..tostring(i))
            else open_record=imgui.Button(record.display_name .. "##lmm_record_" .. tostring(i), 600, 30) end
            if open_record then
    ...
    if log_manager_state.pending_delete ~= "" then
        log_tools.dialog_section(imgui,"永久删除此 TXT？", "Permanently delete this TXT?")
    ...
    log_tools.dialog_section(imgui,"翻页", "Pages")
    if imgui.DisableNext then imgui.DisableNext(log_manager_state.page<=1) end
    ...
    imgui.SameLine()
    if imgui.DisableNext then imgui.DisableNext(log_manager_state.page>=page_count) end
...
-- FlyWithLua macro labels are static: bilingual entries stay understandable
-- after an in-session language change; dialog content follows the global option.
add_macro("StarLux LMM | 设置 / Settings", "ma_open_settings_window()")
add_macro("StarLux LMM | 落地记录 / Landing records", "ma_open_log_manager()")
create_command(
    "starlux/lmm/open_settings",
    "StarLux LMM | 设置 / Settings",
    "ma_open_settings_window()",
    "",
    ""
)
create_command("starlux/lmm/open_records", "StarLux LMM | 落地记录 / Landing records", "ma_open_log_manager()", "", "")
...
function log_tools.apply_rollout_score()
    ...
    r.original_status=r.original_status or landing_status
    landing_status=r:score(landing_status,r.applied_steps)
    r.applied_steps=r.steps;r.applied_revision=r.revision
    rec.summary_dirty=true
    refresh_popup_cache()
end

function log_tools.record_frame(sample)
    local rec=log_tools.recording
    rec.language=runtime_state.document_language
    local ok,err=pcall(function()
        rec:bind_rollout(log_tools.dev.route)
        log_tools.apply_rollout_score()
        local control=log_tools.dev.engine
        for _,axis in ipairs({"pitch","roll","yaw"}) do
            sample[axis.."_input"]=control[axis.."_input_valid"] and control[axis.."_input_ratio"] or nil
            sample[axis.."_source"]=control[axis.."_input_source"]
        end
        for i=1,4 do
            sample["throttle"..i]=control["throttle_"..i.."_valid"] and control["throttle_"..i.."_ratio"] or nil
            sample["n1_"..i]=control["n1_"..i.."_valid"] and control["n1_"..i.."_percent"] or nil
            sample["lever"..i]=control["airbus_lever_"..i.."_valid"] and control["airbus_lever_"..i.."_ratio"] or nil
            sample["detent"..i]=control["airbus_lever_"..i.."_valid"] and control["airbus_detent_"..i] or nil
        end
        for _,surface in ipairs({"elevator","aileron","rudder"}) do
            for i=1,2 do
                local key=surface.."_"..i
                sample[surface..i]=control[key.."_valid"] and control[key.."_deg"] or nil
            end
        end
        sample.detent_mode=control.airbus_mode_valid and control.airbus_mode or nil
        sample.engine_count=log_tools.dev.engine_count
        sample.trace_fpm=sample.physical_fpm or sample.vvi
        sample.trace_fpm_source=sample.physical_fpm and "physical_velocity" or sample.vvi and "vvi" or nil
        if rec.closed and not rec.export and not landing_jobs.log_write and not landing_jobs.log_pending
            and bounce_state.phase~="second_capture" then
            if rec.summary_dirty and rec.summary and rec.id==landing_context.recording_id and not rec.export_failed then
                local ok,built,result=pcall(log_tools.build_landing_log_payload,rec.report_path)
                if ok and built then
                    rec:attach_summary(result.content,rec.report_path);rec.summary_dirty=false
                else rec.error=tostring(ok and result or built);rec.export_failed=true end
            end
            rec:begin_export()
        end
        local previous_id=rec.id
        local route=log_tools.dev.route
        local confirmed
        if sample.ground==1 and route and route.session_id==rec.id and landing_context.runway_detected then
            confirmed=log_tools.ils_module.target(landing_context.airport_id,route.runway,landing_context.runway)
        end
        -- Runway prefetch is available before touchdown; dev.route is confirmed
        -- only afterwards. Keep the approach ILS hypothesis separate from it.
        local ils_ok=pcall(log_tools.ils.update,log_tools.ils,sample,log_tools.runway_state.cache,confirmed)
        if not ils_ok then
            log_tools.ils_module.geometry(nil,sample,log_tools.dev.ils)
            log_tools.dev.ils.status="acquisition_error"
        end
        rec:tick(sample,log_tools.dev.config.wind_interval,route,log_tools.dev.ils.reference)
        log_tools.apply_rollout_score()
        if rec.id~=previous_id then
            log_tools.dev.route=nil
            if confirmed then log_tools.ils.selected_target=nil; log_tools.ils.reference=nil; log_tools.ils.next_select=0 end
        end
        for _,event in ipairs(log_tools.dev.inputs.events) do
            if event.seq>(log_tools.dev.recorded_sequence or 0) then
                if rec.file then rec:emit("E",{event.t,"input_source",event.axis,event.old,event.source,event.mode,event.reason}) end
                log_tools.dev.recorded_sequence=event.seq
            end
        end
        if rec.saved_path and rec.saved_path~=log_tools.dev.last_recording_path then
            log_tools.dev.last_recording_path=rec.saved_path
            log_tools.register_log_filename(file_name_from_path(rec.saved_path))
            if logMsg then logMsg("[StarLux LMM] Recording committed: "..rec.saved_path) end
        end
    end)
    if not ok then
        rec.error=tostring(err)
        if not log_tools.dev.recorder_error_reported and logMsg then logMsg("[StarLux LMM] Recorder: "..tostring(err)); log_tools.dev.recorder_error_reported=true end
        rec:shutdown()
    end
end
...
function ma_landing_meter_update()
    ...
    log_tools.dev:update(now,tostring(AIRCRAFT_PATH or PLANE_ICAO or "unknown") .. "|" .. tostring(AIRCRAFT_FILENAME or "") .. "|" .. tostring(PLANE_TAILNUMBER or ""))
    local action=log_tools.dev.pending
    log_tools.dev.pending=nil
    if action=="settings" then ma_open_settings_window()
    elseif action=="records" then ma_open_log_manager()
    elseif action=="save" then save_settings()
    elseif action=="stop_recording" then log_tools.recording:stop("manual_end",false)
    elseif action=="retry_recording_export" then log_tools.recording.export_failed=false; log_tools.recording.error=nil
    elseif action and action:match("^cycle_") then log_tools.dev:cycle(action:sub(7)) end
    ...
    local frame_period_sec = lmm_get_float("framerate_period_sec")
    ...
    local true_heading = LMM_DATAREFS.heading_deg_true and lmm_get_float("heading_deg_true")
    log_tools.dev:update_wind(now,true_heading,LMM_DATAREFS.heading_deg_mag and heading_deg_mag,
        LMM_DATAREFS.wind_speed_kts and wind_speed_kts,LMM_DATAREFS.wind_heading_deg_mag and wind_heading_deg_mag)

    log_tools.record_frame({t=now,lat=LMM_DATAREFS.latitude_deg and lmm_get_double("latitude_deg"),
        lon=LMM_DATAREFS.longitude_deg and lmm_get_double("longitude_deg"),agl=LMM_DATAREFS.y_agl_m and radio_alt_ft,msl=meters_to_feet(elevation_m),
        ground=LMM_DATAREFS.on_ground and on_ground,gs=LMM_DATAREFS.groundspeed_mps and gs_kt,ias=ias_kts,vy=local_vy_mps,physical_fpm=LMM_DATAREFS.local_vy_mps and local_vy_mps*196.850394,
        vvi=LMM_DATAREFS.vs_fpm and vs_fpm,aoa=LMM_DATAREFS.aoa_deg and aoa_deg,pitch=pitch_deg,roll=roll_deg,
        heading=LMM_DATAREFS.heading_deg_mag and heading_deg_mag,g=LMM_DATAREFS.g_normal and current_g,replay=is_replay,paused=lmm_get_int("paused")~=0,
        true_heading=true_heading,
        wind_speed=LMM_DATAREFS.wind_speed_kts and wind_speed_kts,
        wind_from=LMM_DATAREFS.wind_heading_deg_mag and wind_heading_deg_mag,
        identity=log_tools.dev.inputs.identity})
    ...
    -- 仅在低速地面或平飞巡航阶段，以固定小块渐进建立持久化机场字节索引。
    -- 进近、接地分析、报告写入或跑道解析期间自动暂停，不争用关键帧预算。
    pcall(
        log_tools.update_apt_index,
        now,
        radio_alt_ft,
        on_ground,
        local_vy_mps,
    ...
    -- 触地检测：状态从离地变为接地。
    if armed == true
        and landing_analysis.phase == "idle"
        ...
        begin_bounce_monitor(now, radio_alt_ft)
    ...
    process_bounce_monitor(now, on_ground, radio_alt_ft, local_vy_mps, current_g, gs_kt)
...
function ma_landing_meter_draw()
    ...
    local screen_w = SCREEN_WIDTH or 1920
    local screen_h = SCREEN_HIGHT or 1080
    -- 回放期间不绘制落地自动提示，避免旧提示跨越回放状态残留。
    if runtime_state.replay_active then
        log_tools.native_popup(false, now)
        log_tools.draw_database_status(screen_w, screen_h, now)
        return
    end
    local popup_visible = show_until > 0 and now <= show_until

    if not log_tools.native_popup(popup_visible, now) and popup_visible then
        if landing_report_notice.ready and landing_report_notice.flash_started_at <= 0 then
            log_tools.start_report_flash(now)
            show_until = math.max(show_until, landing_report_notice.flash_until_time)
        end
        local report_flash_active, report_flash_visible = log_tools.report_flash_state(now)
        ...
        local show_centerline_line = popup_cache.centerline_text ~= ""
        local extra_line_count = show_centerline_line and 1 or 0
        ...
        if report_flash_active then
            extra_line_count = extra_line_count + 1
        end

        -- 面板随实际文本宽度和真实行数自适应；字号决定排版，粗细只影响描画次数。
        local font_height, line_gap, bottom_padding, top_padding, minimum_panel_w =
            log_tools.popup_font_metrics(POPUP_LAYOUT)
        local line_count = 7 + extra_line_count
        local maximum_text_w = 0
        for i = 1, 7 do
            maximum_text_w = math.max(maximum_text_w, log_tools.popup_measure_text(popup_cache.lines[i]))
        end
        if show_centerline_line then
            maximum_text_w = math.max(maximum_text_w, log_tools.popup_measure_text(popup_cache.centerline_text))
        end
        if show_bounce_warning then
            maximum_text_w = math.max(maximum_text_w, log_tools.popup_measure_text(popup_cache.bounce_text))
        end
        if show_surface_warning then
            maximum_text_w = math.max(maximum_text_w, log_tools.popup_measure_text(popup_cache.warning_text))
        end
        if report_flash_active then
            maximum_text_w = math.max(maximum_text_w, log_tools.popup_measure_text(landing_report_notice.text))
        end
        local left_padding = POPUP_LAYOUT == "vertical" and 14 or ACCENT_THICKNESS + 10
        local panel_w = math.max(minimum_panel_w, math.ceil(maximum_text_w + left_padding + 16))
        panel_w = math.min(panel_w, math.max(120, screen_w - 20))
        local panel_h = bottom_padding + font_height + (line_count - 1) * line_gap + top_padding

        local x, y = calculate_popup_position(screen_w, screen_h, panel_w, panel_h)
        log_tools.legacy_popup_bounds={x=x,y=y,w=panel_w,h=panel_h,sw=screen_w,sh=screen_h}
        local r, g, b = status_color(landing_status)
        local text_r, text_g, text_b = log_tools.popup_text_color(r, g, b)

        -- 自定义状态底板配合透明度档位；导航查询和文件写入均已移出触地关键阶段。
        XPLMSetGraphicsState(0, 0, 0, 1, 1, 0, 0)
        ...
        local text_x = x + left_padding
        local line_y1 = y + bottom_padding + (line_count - 1) * line_gap

        -- 基础七行与附加状态统一使用所选高对比字体色和粗细。
        glColor4f(text_r, text_g, text_b, 0.98)
        log_tools.draw_popup_text(text_x, line_y1, popup_cache.lines[1])
        log_tools.draw_popup_text(text_x, line_y1 - line_gap, popup_cache.lines[2])
        log_tools.draw_popup_text(text_x, line_y1 - line_gap * 2, popup_cache.lines[3])
        log_tools.draw_popup_text(text_x, line_y1 - line_gap * 3, popup_cache.lines[4])
        log_tools.draw_popup_text(text_x, line_y1 - line_gap * 4, popup_cache.lines[5])
        log_tools.draw_popup_text(text_x, line_y1 - line_gap * 5, popup_cache.lines[6])
        log_tools.draw_popup_text(text_x, line_y1 - line_gap * 6, popup_cache.lines[7])

        local alert_line_index = 7
        if show_centerline_line then
            log_tools.draw_popup_text(
                text_x,
    ...
    -- 普通模式不再绘制底部报告提示；仅 Debug 模式保留完整路径诊断信息。
    if DEBUG_MODE
        and POPUP_MODE ~= "clean"
        and landing_report_notice.debug_until_time > now
        and landing_report_notice.path ~= "" then
        local notice_w = math.min(900, screen_w - 40)
        local notice_h = 36
        ...
        draw_string(
            notice_x + 12,
            notice_y + 12,
            log_tools.ui_text("落地详细报告已生成: ", "Detailed landing report generated: ")
                .. landing_report_notice.path
        )
    end
    -- 最后绘制左上角状态条，避免用户把落地弹窗放在同一区域时完全遮住初始化状态。
    log_tools.draw_database_status(screen_w, screen_h, now)
end

function ma_live_settings() log_tools.dev:legacy_build("settings") end
function ma_live_records() log_tools.dev:legacy_build("records") end
function ma_live_stick() log_tools.dev:legacy_build("stick") end
function ma_live_throttle() log_tools.dev:legacy_build("throttle") end
function ma_live_n1() log_tools.dev:legacy_build("n1") end

-- FlyWithLua's legacy click hook provides down/drag/up and resumes only handled clicks.
function ma_lmm119_popup_mouse()
    local b=log_tools.legacy_popup_bounds
    if not b or runtime_state.replay_active or current_sim_time()>show_until then
        log_tools.legacy_popup_drag=nil; return
    end
    local x,y=tonumber(MOUSE_X),tonumber(MOUSE_Y)
    if not x or not y then return end
    if MOUSE_STATUS=="down" then
        if x<b.x or x>b.x+b.w or y<b.y or y>b.y+b.h then return end
        log_tools.legacy_popup_drag={x=x,y=y,left=b.x,bottom=b.y}
    elseif (MOUSE_STATUS=="drag" or MOUSE_STATUS=="up") and log_tools.legacy_popup_drag then
        local d=log_tools.legacy_popup_drag
        log_tools.dev.config.popup_free_x=math.max(0,math.min(1,(d.left+x-d.x)/math.max(1,b.sw-b.w)))
        log_tools.dev.config.popup_free_y=math.max(0,math.min(1,(d.bottom+y-d.y)/math.max(1,b.sh-b.h)))
        runtime_state.popup_offset_x=0; runtime_state.popup_offset_y=0
        if MOUSE_STATUS=="up" then log_tools.legacy_popup_drag=nil; log_tools.dev.pending="save" end
    else return end
...
function ma_lmm118_shutdown()
    log_tools.ils:close()
    log_tools.recording:shutdown()
    log_tools.native_shutdown()
end
if type(do_on_exit) == "function" then do_on_exit("ma_lmm118_shutdown()") end
