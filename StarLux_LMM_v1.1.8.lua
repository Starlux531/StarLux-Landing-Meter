-- StarLux 落地率插件 v1.1.8
-- 适用于 X-Plane 12.4.4 + FlyWithLua (LuaJIT)，需同时安装 LMM_UI_118 文件夹。
    ...
    for _, key in ipairs({ "pitch", "roll", "yaw" }) do
        ...
        for i = 1, #spec.candidates do
            ...
            if candidate.available then
                if first_available == nil then first_available = candidate end
                if LMM_CONTROL_REFS.input_candidate_is_active(candidate, slots, count) then
                    selected = candidate
                    break
                end
            end
        end
        selected = selected or first_available
        spec.selected_candidate = selected
        if selected ~= nil then
            local primary = spec.candidates[1]
            for i = 1, count do
                local slot = slots[i]
                slot[primary.value_field] = slot[selected.value_field]
                slot[primary.valid_field] = slot[selected.valid_field]
            end
        end
    end
end
...
function LMM_CONTROL_REFS.capture(slot)
...
local function finish_flare_trace(now, reason)
    ...
    -- 连续接地满 2 秒已经覆盖“不弹跳”判定；无需再让无弹跳监视器空等到 6 秒。
    if bounce_state.monitoring == true
        and bounce_state.detected == false
        and bounce_state.phase == "waiting" then
        bounce_state.monitoring = false
...
function log_tools.native_initialize()
    ...
    local ok, result = pcall(function()
        ...
        local directory = join_path(LMM_BASE_DIRECTORY, "LMM_UI_118") .. PATH_SEPARATOR
    ...
    if logMsg then
        logMsg(ui.instance and ui.instance.ready
            and "[StarLux LMM] 1.1.8 SDK 440 Unicode popup ready. " .. (ui.instance.version_info or "")
            or "[StarLux LMM] 1.1.8 legacy popup fallback: " .. ui.error)
    end
end
...
log_tools.report_translations = {
    ...
    { "评分说明: v1.1.8 的拉平曲率与操纵数据仅用于复盘展示，暂不参与评分。", "Rating note: flare curvature and control data in v1.1.8 are for review only and do not affect the rating." },
    ...
    { "StarLux 落地率插件 v1.1.8 - 单次落地记录", "StarLux Landing Meter v1.1.8 - Landing Report" },
...
function log_tools.build_landing_log_payload()
    -- 先把报告组装到内存。实际文件写入由 landing_jobs 在后续帧分块完成。
    -- 先按整段活动度确定输入源；插件接管标准摇杆 DataRef 时，会回退到 cockpit2/FC。
    ...
    file:write("StarLux 落地率插件 v1.1.8 - 单次落地记录\n")
    file:write("======================================================================\n\n")
    ...
    file:write("评分说明: v1.1.8 的拉平曲率与操纵数据仅用于复盘展示，暂不参与评分。\n")
...
if logMsg then
    logMsg(string.format(
        "[StarLux LMM] v1.1.8 loaded successfully with %d direct XPLM DataRefs, persistent apt.dat indexing and optional control capture.",
        #LMM_DATAREF_SPECS
    ))
end
