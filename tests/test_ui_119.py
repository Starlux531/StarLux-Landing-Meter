...
Run: <python> tests/test_ui_119.py
...
def runtime():
    ...
    lua.execute(r'''
        ...
        function api.XPLMCreateWindowEx(spec)
            ...
            calls.specs=calls.specs or {}; calls.specs[calls.windows]=calls.spec
        ...
        function load_native()
            native=assert(loadfile(ROOT..'LMM_UI_119/native_popup.lua'))(ROOT..'LMM_UI_119/')
            host=native.new({system='IBM',root='sim/',assets=ROOT..'LMM_UI_119/'})
        end
        ...
        function paint()
            native_context=true
            local target=ui and ui.native_ui.instance and ui.native_ui.instance.window
            local spec=target and calls.specs[window_index(target)] or calls.spec
            spec.drawWindowFunc(nil,nil)
...
class LayoutTests(unittest.TestCase):
    def test_size_wrap_and_weight_envelope(self):
        l=runtime()
        l.execute('''
            layout=dofile(ROOT..'LMM_UI_119/layout.lua')
    ...
    def test_real_font_coverage_and_license(self):
        ...
        text = (ROOT/'StarLux_LMM_v1.1.9.lua').read_text(encoding='utf-8-sig')
        text += ''.join(p.read_text(encoding='utf-8') for p in (ROOT/'LMM_UI_119').glob('*.lua'))
        needed = {ord(c) for c in text if '\u4e00' <= c <= '\u9fff'}
        for weight in ('Regular','Medium','Bold'):
            f=TTFont(ROOT/f'LMM_UI_119/fonts/LMMUI-{weight}.otf')
            self.assertFalse(needed - set(f.getBestCmap()))
            self.assertEqual(f['OS/2'].usWeightClass, {'Regular':400,'Medium':500,'Bold':700}[weight])
            f.close()
        self.assertIn('SIL OPEN FONT LICENSE', (ROOT/'LMM_UI_119/fonts/OFL.txt').read_text())
...
class RecorderTests(unittest.TestCase):
        ...
        l.execute('''
            ...
            for _,lang in ipairs({'zh','en'}) do
                ...
                for _,w in ipairs({520,860,1200}) do
                    ...
                    for _,page in ipairs({'general','position','colors','fonts','tools','live'}) do
                        ...
                        assert(sections>=(page=='live' and 1 or 2))
    ...
    def load_recorder(self, setup='', cfg='popup_font_size=large\ndocument_language=zh\nrunway_detection_enabled=false\n', source=None, allow_startup_failure=False):
        l=runtime()
        l.globals().MAIN_SOURCE=source or (ROOT/'StarLux_LMM_v1.1.9.lua').read_text(encoding='utf-8-sig')
        ...
        ''')
        l.execute(setup)
        l.execute('assert(loadstring(MAIN_SOURCE))()')
    ...
    def test_explicit_legacy_variant_and_language_defaults(self):
        base=(ROOT/'StarLux_LMM_v1.1.9.lua').read_text(encoding='utf-8-sig')
    ...
    def test_main_migration_language_preview_and_cleanup(self):
        ...
        l.execute('''
            ...
            assert(calls.destroyed_windows==calls.windows and calls.destroyed_fonts==3)
        ''')
    ...
    def test_main_saved_size_bounds_and_compatibility(self):
        for value, expected in [('10',10),('32',32),('999',18),('nan',18),('-1',18)]:
            with self.subTest(value=value):
                ...
                l.execute('ma_landing_meter_draw(); assert(not ui.native_ui.instance.visible)')
                self.assertEqual(l.eval('ui.popup_style.font_px'),expected)

    def test_compile_top_level_and_algorithm_unchanged(self):
        l=runtime()
        for path in ('tests/fixtures/v1.1.8/StarLux_LMM.lua','StarLux_LMM_v1.1.9.lua',
                     'LMM_UI_119/native_popup.lua','LMM_UI_119/layout.lua'):
        ...
        new=(ROOT/'StarLux_LMM_v1.1.9.lua').read_text(encoding='utf-8-sig')
        ...
        def functions(source):
            ...
            for i,m in enumerate(matches):
                ...
                # Function bodies end at their first unindented end. A later
                # top-level bootstrap do/end is not part of that function.
                last=block.find('\nend')
                result[m.group(1)]=block[:last+4]
            return result
        before,after=functions(old),functions(new)
        changed={k for k,v in before.items() if after.get(k)!=v}
        allowed={'calculate_popup_position','log_tools.native_popup','log_tools.popup_language','log_tools.settings_text','save_settings','load_settings',
                 ...
                 'log_tools.write_viewer_html', 'log_tools.native_shutdown',
                 # 1.1.9 observation plumbing; independently exercised in test_live_119.py.
                 'LMM_CONTROL_REFS.capture', 'LMM_CONTROL_REFS.resolve_input_sources',
                 'ma_landing_meter_update', 'begin_landing_context',
                 'log_tools.process_apt_line', 'log_tools.finish_airport_block',
                 'log_tools.build_landing_log_payload', 'log_tools.finish_landing_log_write',
                 'ma_lmm118_shutdown',
                 # beta7: whole-session bounce lifecycle; test_bounce_119.py.
        ...
        normalized=new.replace(version,'1.1.8').replace('LMM_UI_119','LMM_UI_118')
        # Added pause DataRef and retained runway object do not alter touchdown math.
        normalized=normalized.replace(',\n    { key = "heading_deg_true", path = "sim/flightmodel/position/psi", kind = "float" }','')
        normalized=normalized.replace(',\n    { key = "paused", path = "sim/time/paused", kind = "int" }','')
        normalized=normalized.replace('                    geometry = runway,\n','')
        after_norm=functions(normalized)
