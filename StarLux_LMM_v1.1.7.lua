-- StarLux 落地率插件 v1.1.7 测试版
-- 适用于 X-Plane 12 + FlyWithLua
-- v1.1.7 新增 apt.dat 分片持久化字节索引、FPS 自适应安全建库与双语状态提示；落地算法保持不变。
...
function log_tools.popup_language()
    -- FlyWithLua 的 10/18 号 GLUT 字体没有中文字形；非标准字号统一使用完整英文标签，
    -- 避免只剩数字和 ASCII 片段。标准字号继续使用 X-Plane 的 Unicode 字体。
    if log_tools.popup_style.font_size ~= "normal" then return "en" end
...
function log_tools.popup_font_metrics(layout)
    if layout == "vertical" then
        if log_tools.popup_style.font_size == "large" then return 18, 28, 16, 30, 280 end
        if log_tools.popup_style.font_size == "small" then return 10, 21, 14, 26, 235 end
        return 12, 27, 18, 31, 265
    end
    if log_tools.popup_style.font_size == "large" then return 18, 24, 14, 16, 360 end
    if log_tools.popup_style.font_size == "small" then return 10, 15, 12, 16, 330 end
    return 12, 18, 18, 14, 395
end

function log_tools.popup_measure_text(value)
    local ok, width
    if log_tools.popup_style.font_size == "large" then
        ok, width = pcall(measure_string, tostring(value or ""), "Helvetica_18")
    elseif log_tools.popup_style.font_size == "small" then
    ...
    local approximate_size = log_tools.popup_style.font_size == "large" and 18
        or (log_tools.popup_style.font_size == "small" and 10 or 12)
...
log_tools.report_translations = {
    ...
    { "评分说明: v1.1.7 的拉平曲率与操纵数据仅用于复盘展示，暂不参与评分。", "Rating note: flare curvature and control data in v1.1.7 are for review only and do not affect the rating." },
    ...
    { "StarLux 落地率插件 v1.1.7 - 单次落地记录", "StarLux Landing Meter v1.1.7 - Landing Report" },
...
function log_tools.build_landing_log_payload()
    ...
    file:write("StarLux 落地率插件 v1.1.7 - 单次落地记录\n")
    file:write("======================================================================\n\n")
    ...
    file:write("评分说明: v1.1.7 的拉平曲率与操纵数据仅用于复盘展示，暂不参与评分。\n")
...
function ma_build_settings_window(wnd, x, y)
    ...
    imgui.TextUnformatted(log_tools.settings_text("关闭后仍保留 1.1.7 落地算法，不使用磁航向猜测跑道。", "Off keeps the v1.1.7 landing algorithm and never guesses a runway from heading."))
    ...
    imgui.TextUnformatted(log_tools.settings_text("位置微调（每次 5 像素，不改变九宫格锚点）", "Fine position (5 px per click; keeps the nine-position anchor)"))
    ...
    imgui.Separator()
    imgui.TextUnformatted(log_tools.settings_text("各等级弹窗颜色", "Popup colour by rating"))
    if imgui.RadioButton("Nice##lmm_colour_nice", log_tools.popup_style.edit_status == "NICE") then log_tools.popup_style.edit_status = "NICE" end
    imgui.SameLine()
    if imgui.RadioButton("Stable##lmm_colour_stable", log_tools.popup_style.edit_status == "STABLE") then log_tools.popup_style.edit_status = "STABLE" end
    imgui.SameLine()
    if imgui.RadioButton("Attention##lmm_colour_attention", log_tools.popup_style.edit_status == "ATTENTION") then log_tools.popup_style.edit_status = "ATTENTION" end
    imgui.SameLine()
    if imgui.RadioButton("UNSTABLE##lmm_colour_unstable", log_tools.popup_style.edit_status == "UNSTABLE") then log_tools.popup_style.edit_status = "UNSTABLE" end

    local edit_color = log_tools.popup_style.colors[log_tools.popup_style.edit_status]
    local red_changed, new_red = imgui.SliderFloat("Red##lmm_popup_red", edit_color[1], 0, 1, "Red %.2f")
    local green_changed, new_green = imgui.SliderFloat("Green##lmm_popup_green", edit_color[2], 0, 1, "Green %.2f")
    local blue_changed, new_blue = imgui.SliderFloat("Blue##lmm_popup_blue", edit_color[3], 0, 1, "Blue %.2f")
    ...
    imgui.Button(
        string.format("%s RGB %.2f / %.2f / %.2f##lmm_colour_preview", log_tools.popup_style.edit_status, preview_red, preview_green, preview_blue),
        330,
        30
    )
    imgui.PopStyleColor()
    ...
    imgui.TextUnformatted(log_tools.settings_text("字体颜色", "Popup text colour"))
    ...
    imgui.TextUnformatted(log_tools.settings_text("字体大小（同步调整弹窗尺寸）", "Font size (also adjusts popup dimensions)"))
    ...
    imgui.TextUnformatted(log_tools.settings_text(
        "标准字号保留中文；小号/大号受 FlyWithLua 字体限制，弹窗使用完整英文标签。",
        "Normal preserves Chinese. Small/Large use complete English labels because FlyWithLua's native fonts do not contain Chinese glyphs."
    ))

    imgui.TextUnformatted(log_tools.settings_text("字体粗细（不改变弹窗尺寸）", "Font weight (does not change popup dimensions)"))
...
if logMsg then
    logMsg(string.format(
        "[StarLux LMM] v1.1.7 loaded successfully with %d direct XPLM DataRefs, persistent apt.dat indexing and optional control capture.",
        #LMM_DATAREF_SPECS
    ))
end
